...
impl Spawn {
	...
	pub(crate) fn stdout_regex(self, expected_stdout: impl AsRef<str>) -> Self {
		Self { expected_stdout: Expected::regex(expected_stdout.as_ref()), ..self }
	}
}
...
impl CommandBuilder {
	...
	pub(crate) fn core(self, core: &mockcore::Handle) -> Self {
		Self { core_url: Some(core.url()), core_cookie_file: Some(core.cookie_file()), ..self }
	}

	pub(crate) fn ord(self, ord: &TestServer) -> Self {
		Self { ord_url: Some(ord.url()), ..self }
	}
	...
	pub(crate) fn expected_stderr(self, expected_stderr: impl AsRef<str>) -> Self {
		Self { expected_stderr: Expected::String(expected_stderr.as_ref().to_owned()), ..self }
	}

	pub(crate) fn stderr_regex(self, expected_stderr: impl AsRef<str>) -> Self {
		Self { expected_stderr: Expected::regex(expected_stderr.as_ref()), ..self }
	}

	pub(crate) fn expected_exit_code(self, expected_exit_code: i32) -> Self {
		Self { expected_exit_code, ..self }
	}
	...
	pub(crate) fn command(&self) -> Command {
		...
		command
			.stdin(Stdio::piped())
			.stdout(if self.stdout { Stdio::piped() } else { Stdio::inherit() })
			.stderr(if self.stderr { Stdio::piped() } else { Stdio::inherit() })
	...
	pub(crate) fn spawn(self) -> Spawn {
		...
		child.stdin.as_ref().unwrap().write_all(&self.stdin).unwrap();
