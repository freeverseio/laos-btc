...
use ord::subcommand::wallet::send::Output;
...
fn address_page_shows_outputs_and_sat_balance() {
	...
	ord.assert_response_regex(
		...
		format!(
			...
			OutPoint { txid: send.txid, vout: 0 }
		),
	);
}
...
fn inscription_page() {
	...
	let response = ord.json_request(format!("/output/{}", OutPoint { txid: reveal, vout: 0 }));
	...
	TestServer::spawn_with_args(&core, &[]).assert_html(
		...
		InscriptionHtml {
			...
			rune: None,
			sat: None,
			satpoint: SatPoint { outpoint: OutPoint { txid: reveal, vout: 0 }, offset: 0 },
			timestamp: "1970-01-01 00:00:02+00:00".parse::<DateTime<Utc>>().unwrap(),
		},
	);
}
...
fn multiple_inscriptions_appear_on_reveal_transaction_page() {
	...
	ord.assert_response_regex(
		format!("/tx/{reveal}"),
		format!(
			".*<h1>Transaction .*</h1>.*<a href=/inscription/{id0}.*<a href=/inscription/{id1}.*"
		),
	);
}
...
fn inscription_metadata() {
	...
	let cbor_map = ciborium::value::Value::Map(vec![
		(ciborium::value::Value::Text("foo".into()), ciborium::value::Value::Text("bar".into())),
		(
	...
	assert_eq!(response.text().unwrap(), format!("\"{}\"", hex::encode(encoded_metadata)));
}
...
fn recursive_inscription_endpoint() {
	...
	pretty_assert_eq!(
		...
		api::InscriptionRecursive {
			...
			satpoint: SatPoint { outpoint: inscription.location.outpoint, offset: 0 },
			timestamp: 2,
			value: Some(10000),
			address: None,
		}
...
fn all_endpoints_in_recursive_directory_return_json() {
	...
	assert_eq!(ord_server.request("/r/blockheight").json::<u64>().unwrap(), 2);
	...
	assert_eq!(
		ord_server.request("/r/blockhash/0").json::<String>().unwrap(),
...
fn sat_recursive_endpoints_without_sat_index_return_404() {
	...
	assert_eq!(server.request("/r/sat/5000000000").status(), StatusCode::NOT_FOUND,);

	assert_eq!(server.request("/r/sat/5000000000/at/1").status(), StatusCode::NOT_FOUND,);
}
...
fn inscription_transactions_are_stored_with_transaction_index() {
	...
	assert_eq!(ord.request(format!("/tx/{reveal}")).status(), StatusCode::OK,);

	assert_eq!(ord.request(format!("/tx/{coinbase}")).status(), StatusCode::OK,);

	core.clear_state();

	assert_eq!(ord.request(format!("/tx/{reveal}")).status(), StatusCode::OK,);

	assert_eq!(ord.request(format!("/tx/{coinbase}")).status(), StatusCode::NOT_FOUND,);
}
...
fn run_no_sync() {
	...
	let builder =
		CommandBuilder::new(format!("server --no-sync --address 127.0.0.1 --http-port {port}",))
			.core(&core)
			.temp_dir(tempdir);
...
fn ctrl_c() {
	...
	assert_eq!(buffer, "Shutting down gracefully. Press <CTRL-C> again to shutdown immediately.\n");
