...
use self::{command_builder::CommandBuilder, expected::Expected, test_server::TestServer};
use bitcoin::{
...
use ord::{
	...
	templates::InscriptionHtml,
	wallet::{batch, ListDescriptorsResult},
	Inscription, InscriptionId, RuneEntry,
};
use ordinals::{
	brc721::register_collection::RegisterCollection, Artifact, Brc721CollectionId, Charm, Edict,
	Pile, Rarity, Rune, RuneId, Runestone, Sat, SatPoint, SpacedRune, COIN_VALUE,
};
...
fn inscribe_with_postage(
	...
	let mut command_str =
		format!("--chain {} wallet inscribe --fee-rate 1 --file foo.txt", core.network());
...
fn batch(core: &mockcore::Handle, ord: &TestServer, batchfile: batch::File) -> Etched {
	...
	let id = RuneId { block: block_height, tx: 1 };

	let reveal = output.reveal;
	let parent = output.inscriptions[0].id;

	let batch::Etching { divisibility, premine, rune, supply, symbol, terms, turbo } =
	...
	if let Some(terms) = terms {
		...
		{
			let relative =
				terms.offset.and_then(|range| range.start).map(|start| start + block_height);
		...
		{
			let relative = terms.offset.and_then(|range| range.end).map(|end| end + block_height);
...
.*",
	...
	let batch::RuneInfo { destination, location, rune: _ } = output.rune.clone().unwrap();

	if premine.to_integer(divisibility).unwrap() > 0 {
		let destination = destination.unwrap().clone().require_network(Network::Regtest).unwrap();
...
fn envelope(payload: &[&[u8]]) -> Witness {
	...
	let script = builder.push_opcode(bitcoin::opcodes::all::OP_ENDIF).into_script();
