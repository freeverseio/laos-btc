			...
			runes: [(
				...
				[
					(
						...
						Pile { amount: 499, divisibility: 1, symbol: Some('¢') }
					),
					(
						OutPoint { txid: output.txid, vout: 2 },
						Pile { amount: 501, divisibility: 1, symbol: Some('¢') }
					)
				]
				.into()
			),]
	...
	pretty_assert_eq!(
		...
		Balances {
			runes: [(
				...
				(0..10)
					.map(|i| (
						OutPoint { txid: output.txid, vout: 1 + i },
						Pile { amount: 1000000000, divisibility: 0, symbol: Some('¢') }
					),)
					.collect()
			)]
			.into(),
