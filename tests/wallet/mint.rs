...
use super::*;
use ord::{decimal::Decimal, subcommand::wallet::mint};
...
fn minting_rune_and_fails_if_after_end() {
	...
	pretty_assert_eq!(output.pile, Pile { amount: 1111, divisibility: 1, symbol: Some('¢') });
	...
	pretty_assert_eq!(
		...
		ord::subcommand::balances::Output {
			runes: vec![(
				output.rune,
				vec![(OutPoint { txid: output.mint, vout: 1 }, output.pile,)]
					.into_iter()
					.collect()
			),]
			.into_iter()
...
fn minting_rune_and_then_sending_works() {
	...
	batch(
		...
		batch::File {
			etching: Some(batch::Etching {
				...
				terms: Some(batch::Terms {
					cap: 1,
					offset: Some(batch::Range { end: Some(10), start: None }),
	...
	assert_eq!(
		*balance.runes.unwrap().first_key_value().unwrap().1,
		Decimal { value: 132, scale: 0 }
	);

	assert_eq!(balance.runic.unwrap(), 20000);

	pretty_assert_eq!(output.pile, Pile { amount: 21, divisibility: 0, symbol: Some('¢') });
...
fn minting_rune_with_destination() {
	...
	let destination: Address<NetworkUnchecked> =
		"bcrt1qs758ursh4q9z627kt3pp5yysm78ddny6txaqgw".parse().unwrap();
	...
	assert_eq!(
		...
		ord::subcommand::balances::Output {
			runes: vec![(
				...
				vec![(
					OutPoint { txid: output.mint, vout: 1 },
					Pile { amount: 21, divisibility: 0, symbol: Some('¢') },
				)]
				.into_iter()
				.collect()
			)]
	...
	batch(
		...
		batch::File {
			etching: Some(batch::Etching {
				...
				terms: Some(batch::Terms {
					cap: 1,
					offset: Some(batch::Range { end: None, start: Some(1) }),
					amount: "111.1".parse().unwrap(),
					height: None,
				}),
				turbo: false,
