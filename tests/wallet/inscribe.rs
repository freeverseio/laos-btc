...
use super::*;
use ord::subcommand::wallet::{create, inscriptions, receive};
use std::ops::Deref;
...
fn inscribe_fails_if_bitcoin_core_is_too_old() {
	...
	CommandBuilder::new("wallet inscribe --file hello.txt --fee-rate 1")
		...
		.expected_stderr(
			"error: Bitcoin Core 25.0.0 or newer required, current version is 24.0.0\n",
		)
		.core(&core)
		.ord(&ord)
...
fn refuse_to_inscribe_already_inscribed_utxo() {
	...
	let output = OutPoint { txid: reveal, vout: 0 };
...
fn inscribe_with_fee_rate() {
	...
	pretty_assert_eq!(fee_rate, 2.0);
	assert_eq!(
		ord::FeeRate::try_from(2.0).unwrap().fee(tx1.vsize() + tx2.vsize()).to_sat(),
		output.total_fees
	);
}
...
fn inscribe_with_parent_inscription_and_fee_rate() {
	...
	ord.assert_response_regex(
		...
		format!(".*<dt>children</dt>.*<a href=/inscription/{}>.*", child_output.inscriptions[0].id),
	);
...
fn try_reinscribe_without_flag() {
	...
	.ord(&ord)
	.expected_exit_code(1)
	.stderr_regex(format!("error: sat at {reveal_txid}:0:0 already inscribed.*"))
	.run_and_extract_stdout();
}
...
fn inscribe_can_compress() {
	...
	let client = reqwest::blocking::Client::builder().brotli(false).build().unwrap();

	let response = client
		.get(ord.url().join(format!("/content/{inscription}",).as_ref()).unwrap())
		.send()
		.unwrap();
	...
	let client = reqwest::blocking::Client::builder().brotli(true).build().unwrap();
...
fn inscribe_with_sat_arg_fails_if_no_index_or_not_found() {
	...
	CommandBuilder::new(
		"--index-sats wallet inscribe --sat 5000000000 --file foo.txt --fee-rate 1",
	)
	.write("foo.txt", "FOO")
	.core(&core)
	.ord(&TestServer::spawn_with_server_args(&core, &["--index-sats"], &[]))
	.expected_exit_code(1)
...
fn server_can_decompress_brotli() {
	...
	let response = client
		.get(test_server.url().join(format!("/content/{inscription}",).as_ref()).unwrap())
		.send()
		.unwrap();
...
fn file_inscribe_with_only_delegate() {
	...
	let inscribe =
		CommandBuilder::new(format!("wallet inscribe --fee-rate 1.0 --delegate {delegate}"))
			.core(&core)
			.ord(&ord)
			.run_and_deserialize_output::<Batch>();
