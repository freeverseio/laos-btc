...
use super::*;
use ord::subcommand::wallet::create;
...
fn restore_generates_same_descriptors() {
	let (mnemonic, descriptors) = {
		...
		let create::Output { mnemonic, .. } =
			CommandBuilder::new("wallet create").core(&core).run_and_deserialize_output();
...
fn restore_generates_same_descriptors_with_passphrase() {
	...
	CommandBuilder::new(["wallet", "restore", "--passphrase", passphrase, "--from", "mnemonic"])
		.stdin(mnemonic.to_string().into())
		.core(&core)
		.run_and_extract_stdout();
...
fn timestamp_conflicts_with_descriptor() {
	...
	CommandBuilder::new(["wallet", "restore", "--from", "descriptor", "--timestamp", "now"])
		.stdin("".into())
		.core(&core)
		.ord(&ord)
		.expected_exit_code(1)
...
fn restore_with_now_timestamp() {
	...
	CommandBuilder::new(["wallet", "restore", "--from", "mnemonic", "--timestamp", "now"])
		.stdin(mnemonic.to_string().into())
		.core(&core)
		.run_and_extract_stdout();
	...
	assert!(output.descriptors.iter().all(|descriptor| match descriptor.timestamp {
...
fn restore_with_timestamp() {
	...
	CommandBuilder::new(["wallet", "restore", "--from", "mnemonic", "--timestamp", "123456789"])
		.stdin(mnemonic.to_string().into())
		.core(&core)
		.run_and_extract_stdout();
