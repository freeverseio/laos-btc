...
use super::*;
use ord::{decimal::Decimal, subcommand::wallet::send};
...
fn batch_inscribe_can_create_one_inscription() {
	...
	assert_eq!(request.headers().get("content-type").unwrap(), "text/plain;charset=utf-8");
...
fn batch_in_same_output_but_different_satpoints() {
	...
	for (i, inscription) in output.inscriptions.iter().enumerate() {
		assert_eq!(
			...
			SatPoint { outpoint, offset: u64::try_from(i).unwrap() * 10_000 }
		);
	}
...
fn batch_in_same_output_with_non_default_postage() {
	...
	for (i, inscription) in output.inscriptions.iter().enumerate() {
		assert_eq!(
			inscription.location,
			SatPoint { outpoint, offset: u64::try_from(i).unwrap() * 777 }
		);
	}
...
"
	...
	ord.assert_response_regex(
		...
		format!(
			r#".*<dt>delegate</dt>\s*<dd><a href=/inscription/{delegate}>{delegate}</a></dd>.*"#,
		),
	);
...
fn batch_inscribe_with_satpoints_with_parent() {
	...
	let satpoint_1 = SatPoint { outpoint: OutPoint { txid: txids[0], vout: 0 }, offset: 0 };

	let satpoint_2 = SatPoint { outpoint: OutPoint { txid: txids[1], vout: 0 }, offset: 0 };

	let satpoint_3 = SatPoint { outpoint: OutPoint { txid: txids[2], vout: 0 }, offset: 0 };

	let sat_1 = serde_json::from_str::<api::Output>(
		&ord.json_request(format!("/output/{}", satpoint_1.outpoint)).text().unwrap(),
	)
	.unwrap()
	.sat_ranges
	.unwrap()[0]
	...
	let sat_2 = serde_json::from_str::<api::Output>(
		&ord.json_request(format!("/output/{}", satpoint_2.outpoint)).text().unwrap(),
	)
	.unwrap()
	.sat_ranges
	.unwrap()[0]
	...
	let sat_3 = serde_json::from_str::<api::Output>(
		&ord.json_request(format!("/output/{}", satpoint_3.outpoint)).text().unwrap(),
	)
	.unwrap()
	.sat_ranges
	.unwrap()[0]
...
fn batch_inscribe_with_satpoints_with_different_sizes() {
	...
	let satpoint_1 = SatPoint { outpoint: outpoint_1, offset: 0 };

	let satpoint_2 = SatPoint { outpoint: outpoint_2, offset: 0 };

	let satpoint_3 = SatPoint { outpoint: outpoint_3, offset: 0 };
...
fn batch_can_etch_rune() {
	...
	let rune = SpacedRune { rune: Rune(RUNE), spacers: 0 };
	...
	assert_eq!(reveal.output[pointer].script_pubkey, destination.script_pubkey(),);
	...
	assert_eq!(
		...
		Balance {
			...
			ordinal: 10000,
			runic: Some(10000),
			runes: Some(vec![(rune, Decimal { value: 1000, scale: 0 })].into_iter().collect()),
		...
		batch::File {
			etching: Some(batch::Etching {
				...
				terms: Some(batch::Terms {
					cap: 9,
					amount: "1000".parse().unwrap(),
					offset: Some(batch::Range { start: Some(10), end: Some(20) }),
					height: None,
				}),
				turbo: false,
			}),
		...
		batch::File {
			etching: Some(batch::Etching {
				...
				terms: Some(batch::Terms {
					cap: 9,
					amount: "1000".parse().unwrap(),
					height: Some(batch::Range { start: Some(10), end: Some(20) }),
					offset: None,
				}),
				turbo: false,
			}),
	...
	CommandBuilder::new("--regtest --index-runes wallet batch --fee-rate 0 --batch batch.yaml")
		...
		.write(
			...
			serde_yaml::to_string(&batch::File {
				etching: Some(batch::Etching {
					divisibility: 0,
					rune: SpacedRune { rune: Rune(RUNE), spacers: 1 },
					supply: "1000".parse().unwrap(),
					premine: "1000".parse().unwrap(),
					symbol: '¢',
					terms: None,
	...
	CommandBuilder::new("--regtest --index-runes wallet batch --fee-rate 0 --batch batch.yaml")
		...
		.write(
			...
			serde_yaml::to_string(&batch::File {
				etching: Some(batch::Etching {
					divisibility: 0,
					rune: SpacedRune { rune: Rune::reserved(0, 0), spacers: 0 },
					premine: "1000".parse().unwrap(),
					supply: "1000".parse().unwrap(),
					symbol: '¢',
					terms: None,
	...
	CommandBuilder::new("--regtest --index-runes wallet batch --fee-rate 0 --batch batch.yaml")
		...
		.write(
			...
			serde_yaml::to_string(&batch::File {
				etching: Some(batch::Etching {
					divisibility: 0,
					rune: SpacedRune { rune: Rune(0), spacers: 0 },
					supply: "1000".parse().unwrap(),
					premine: "1000".parse().unwrap(),
					symbol: '¢',
					terms: None,
		...
		.write(
			...
			serde_yaml::to_string(&batch::File {
				etching: Some(batch::Etching {
					...
					terms: Some(batch::Terms {
						cap: 2,
						offset: Some(batch::Range { end: Some(2), start: None }),
		...
		.write(
			...
			serde_yaml::to_string(&batch::File {
				etching: Some(batch::Etching {
					...
					terms: Some(batch::Terms {
						cap: 1,
						offset: Some(batch::Range { end: Some(2), start: Some(2) }),
						amount: "1".parse().unwrap(),
						height: None,
					}),
					turbo: false,
		...
		.write(
			...
			serde_yaml::to_string(&batch::File {
				etching: Some(batch::Etching {
					...
					terms: Some(batch::Terms {
						cap: 1,
						height: Some(batch::Range { end: Some(2), start: Some(2) }),
						amount: "1".parse().unwrap(),
						offset: None,
					}),
					turbo: false,
	...
	.write(
		...
		serde_yaml::to_string(&batch::File {
			etching: Some(batch::Etching {
				...
				terms: Some(batch::Terms {
					cap: 1,
					height: Some(batch::Range { start: Some(u64::MAX - 1), end: Some(u64::MAX) }),
					offset: Some(batch::Range { start: Some(u64::MAX - 1), end: Some(u64::MAX) }),
					amount: "1".parse().unwrap(),
				}),
				turbo: true,
			}),
			inscriptions: vec![batch::Entry { file: Some("inscription.txt".into()), ..default() }],
			..default()
		})
		.unwrap(),
	)
