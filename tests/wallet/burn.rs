...
fn runic_outputs_are_protected() {
	...
	CommandBuilder::new(format!("--regtest --index-runes wallet burn --fee-rate 1 {inscription}",))
		.core(&core)
		.ord(&ord)
...
fn json_metadata_can_be_included_when_burning() {
	...
	ord.assert_html(
		...
		InscriptionHtml {
			...
			fee: 138,
			id: inscription,
			output: Some(TxOut { value: Amount::from_sat(1), script_pubkey }),
			height: 3,
			...
			satpoint: SatPoint { outpoint: OutPoint { txid: output.txid, vout: 0 }, offset: 0 },
			timestamp: "1970-01-01 00:00:03+00:00".parse::<DateTime<Utc>>().unwrap(),
			..default()
		},
	);
}
...
fn burn_rune() {
	...
	pretty_assert_eq!(
		...
		Balance {
			...
			runic: Some(9871),
			runes: Some(
				[(SpacedRune { rune, spacers: 0 }, Decimal { value: 500, scale: 0 })]
					.into_iter()
					.collect()
			),
...
fn burn_rune_with_many_assets_in_wallet() {
	...
	pretty_assert_eq!(
		...
		Balance {
			...
			runes: Some(
				[
					(SpacedRune { rune: rune_0, spacers: 0 }, Decimal { value: 1000, scale: 0 }),
					(SpacedRune { rune: rune_1, spacers: 0 }, Decimal { value: 1000, scale: 0 }),
					(SpacedRune { rune: rune_2, spacers: 0 }, Decimal { value: 1000, scale: 0 })
				]
				.into_iter()
				.collect()
			),
			...
			runes: [
				(
					...
					[(
						...
						Pile { amount: 500, divisibility: 0, symbol: Some('¢') },
					)]
					.into()
				),
				(
