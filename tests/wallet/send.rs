	...
	pretty_assert_eq!(
		...
		api::Output {
			...
			inscriptions: Some(vec![
				InscriptionId { txid: reveal_txid, index: 0 },
				InscriptionId { txid: reveal_txid, index: 1 },
				InscriptionId { txid: reveal_txid, index: 2 },
			]),
			indexed: true,
			runes: None,
...
fn send_btc_locks_inscriptions() {
	...
	assert!(core.get_locked().contains(&OutPoint { txid: reveal, vout: 0 }))
}
	...
	pretty_assert_eq!(
		...
		ord::subcommand::balances::Output {
			runes: vec![(
				...
				vec![(
					OutPoint { txid: output.txid, vout: 0 },
	...
	pretty_assert_eq!(
		...
		ord::subcommand::balances::Output {
			runes: [(
				...
				[
					(
						OutPoint { txid: output.txid, vout: 1 },
						Pile { amount: 223, divisibility: 0, symbol: Some('¢') },
					),
					(
						OutPoint { txid: output.txid, vout: 2 },
						Pile { amount: 777, divisibility: 0, symbol: Some('¢') },
					)
				]
				.into()
			)]
		...
		ord::subcommand::balances::Output {
			runes: vec![(
				...
				vec![
					(
						OutPoint { txid: output.txid, vout: 1 },
						Pile { amount: 9899, divisibility: 1, symbol: Some('¢') },
					),
					(
						OutPoint { txid: output.txid, vout: 2 },
						Pile { amount: 101, divisibility: 1, symbol: Some('¢') },
					)
				]
				.into_iter()
				.collect()
		...
		ord::subcommand::balances::Output {
			runes: vec![(
				...
				vec![
					(
						OutPoint { txid: output.txid, vout: 1 },
						Pile { amount: 250, divisibility: 0, symbol: Some('¢') },
					),
					(
						OutPoint { txid: output.txid, vout: 2 },
						Pile { amount: 750, divisibility: 0, symbol: Some('¢') },
					)
				]
				.into_iter()
				.collect()
...
fn sending_rune_creates_transaction_with_expected_runestone() {
	...
	pretty_assert_eq!(
		...
		Artifact::Runestone(Runestone {
			pointer: None,
			etching: None,
			edicts: vec![Edict { id: etch.id, amount: 750, output: 2 }],
			mint: None,
		}),
	);
}
