...
use ord::{subcommand::wallet::send::Output, Envelope, Inscription};
...
fn get_inscriptions_in_block() {
	...
	let _ = core
		.broadcast_tx(TransactionTemplate { inputs: &[(6, 0, 0, envelope.clone())], ..default() });
...
fn get_output() {
	...
	assert!(!serde_json::from_str::<api::Output>(&response.text().unwrap()).unwrap().indexed);
	...
	pretty_assert_eq!(
		output_json,
		api::Output {
			address: Some("bc1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq9e75rs".parse().unwrap()),
...
fn get_status() {
	...
	let dummy_started = "2012-12-12 12:12:12+00:00".parse::<DateTime<Utc>>().unwrap();
...
fn get_runes() {
	...
	pretty_assert_eq!(
		...
		api::Rune {
			...
			mintable: false,
			parent: Some(InscriptionId { txid: a.output.reveal, index: 0 }),
		}
	);
			...
			entries: vec![
				(
					...
					RuneEntry {
						...
						number: 2,
						premine: 1000,
						spaced_rune: SpacedRune { rune: Rune(RUNE + 2), spacers: 0 },
						symbol: Some('¢'),
						timestamp: 24,
						turbo: false,
					}
...
fn get_decode_tx() {
	...
	let txid = core
		.broadcast_tx(TransactionTemplate { inputs: &[(1, 0, 0, envelope.clone())], ..default() });
	...
	assert_eq!(
		...
		api::Decode { inscriptions, runestone }
	);
}
...
fn outputs_address() {
	...
	pretty_assert_eq!(
		...
		vec![api::Output {
			...
			inscriptions: Some(vec![]),
			outpoint: OutPoint { txid: cardinal_send.txid, vout: 0 },
			indexed: true,
			runes: Some(BTreeMap::new()),
			sat_ranges: None,
			script_pubkey: ScriptBuf::from(
				address.parse::<Address<NetworkUnchecked>>().unwrap().assume_checked()
			),
			spent: false,
	...
	let runes_json: Vec<api::Output> =
		serde_json::from_str(&runes_response.text().unwrap()).unwrap();
	...
	pretty_assert_eq!(
		...
		vec![api::Output {
			address: Some(address.parse().unwrap()),
			inscriptions: Some(vec![]),
			outpoint: OutPoint { txid: rune_send.txid, vout: 0 },
			indexed: true,
	...
	pretty_assert_eq!(
		...
		vec![api::Output {
			address: Some(address.parse().unwrap()),
			inscriptions: Some(vec![InscriptionId { txid: reveal, index: 0 },]),
			outpoint: OutPoint { txid: inscription_send.txid, vout: 0 },
			indexed: true,
			runes: Some(BTreeMap::new()),
			sat_ranges: None,
			script_pubkey: ScriptBuf::from(
	...
	let any: Vec<api::Output> = serde_json::from_str(
		&ord.json_request(format!("/outputs/{}?type=any", address)).text().unwrap(),
	)
	.unwrap();

	let default: Vec<api::Output> =
		serde_json::from_str(&ord.json_request(format!("/outputs/{}", address)).text().unwrap())
			.unwrap();

	assert_eq!(any.len(), 3);
	assert!(any.iter().any(|output| output.runes.clone().unwrap_or_default().len() == 1));
	assert!(any
		.iter()
		.any(|output| output.inscriptions.clone().unwrap_or_default().len() == 1));
	assert!(any
		.iter()
		.any(|output| output.inscriptions.clone().unwrap_or_default().is_empty() &&
...
fn outputs_address_returns_400_for_missing_indices() {
	...
	let ord = TestServer::spawn_with_args(
		&core,
		&["--no-index-inscriptions", "--index-runes", "--index-addresses", "--regtest"],
	);
