...
mod tests {
	...
	fn reveal_script_chunks_metadata() {
		assert_eq!(
			Inscription { metadata: None, ..default() }
				.append_reveal_script(script::Builder::new())
				.instructions()
				.count(),
			4
		...
		assert_eq!(
			Inscription { metadata: Some(Vec::new()), ..default() }
				.append_reveal_script(script::Builder::new())
				.instructions()
				.count(),
			4
		...
		assert_eq!(
			Inscription { metadata: Some(vec![0; 1]), ..default() }
				.append_reveal_script(script::Builder::new())
				.instructions()
				.count(),
			6
		...
		assert_eq!(
			Inscription { metadata: Some(vec![0; 520]), ..default() }
				.append_reveal_script(script::Builder::new())
				.instructions()
				.count(),
			6
		...
		assert_eq!(
			Inscription { metadata: Some(vec![0; 521]), ..default() }
				.append_reveal_script(script::Builder::new())
				.instructions()
				.count(),
			8
	...
	fn inscription_with_no_parent_field_has_no_parent() {
		assert!(Inscription { parents: Vec::new(), ..default() }.parents().is_empty());
	}

	#[test]
	fn inscription_with_parent_field_shorter_than_txid_length_has_no_parent() {
		assert!(Inscription { parents: vec![Vec::new()], ..default() }.parents().is_empty());
	}

	#[test]
	fn inscription_with_parent_field_longer_than_txid_and_index_has_no_parent() {
		assert!(Inscription { parents: vec![vec![1; 37]], ..default() }.parents().is_empty());
	}
	...
	fn inscription_with_parent_field_index_with_trailing_zeroes_and_fixed_length_has_parent() {
		...
		assert!(!Inscription { parents: vec![parent], ..default() }.parents().is_empty());
	}
	...
	fn inscription_parent_with_zero_byte_index_field_is_deserialized_correctly() {
		assert_eq!(
			Inscription { parents: vec![vec![1; 32]], ..default() }.parents(),
	...
	fn metadata_function_decodes_metadata() {
		assert_eq!(
			Inscription { metadata: Some(vec![0x44, 0, 1, 2, 3]), ..default() }
				.metadata()
				.unwrap(),
	...
	fn metadata_function_returns_none_if_no_metadata() {
		assert_eq!(Inscription { metadata: None, ..default() }.metadata(), None,);
	}

	#[test]
	fn metadata_function_returns_none_if_metadata_fails_to_parse() {
		assert_eq!(Inscription { metadata: Some(vec![0x44]), ..default() }.metadata(), None,);
	}

	#[test]
	fn pointer_decode() {
		assert_eq!(Inscription { pointer: None, ..default() }.pointer(), None);
		assert_eq!(Inscription { pointer: Some(vec![0]), ..default() }.pointer(), Some(0),);
		assert_eq!(
			Inscription { pointer: Some(vec![1, 2, 3, 4, 5, 6, 7, 8]), ..default() }.pointer(),
			Some(0x0807060504030201),
		);
		assert_eq!(
			Inscription { pointer: Some(vec![1, 2, 3, 4, 5, 6]), ..default() }.pointer(),
			Some(0x0000060504030201),
		);
		assert_eq!(
			Inscription { pointer: Some(vec![1, 2, 3, 4, 5, 6, 7, 8, 0, 0, 0, 0, 0]), ..default() }
				.pointer(),
			Some(0x0807060504030201),
		);
		assert_eq!(
			Inscription { pointer: Some(vec![1, 2, 3, 4, 5, 6, 7, 8, 0, 0, 0, 0, 1]), ..default() }
				.pointer(),
			None,
		);
		assert_eq!(
			Inscription { pointer: Some(vec![1, 2, 3, 4, 5, 6, 7, 8, 1]), ..default() }.pointer(),
			None,
		);
	}
	...
	#[test]
	fn pointer_encode() {
		assert_eq!(Inscription { pointer: None, ..default() }.to_witness(), envelope(&[b"ord"]),);

		assert_eq!(
			Inscription { pointer: Some(vec![1, 2, 3]), ..default() }.to_witness(),
	...
	fn hidden() {
		...
		case(
			Some("text/html"),
			Some(
				"  /content/09a8d837ec0bcaec668ecf405e696a16bee5990863659c224ff888fb6f8f45e7i0  \n",
			),
			true,
		);
		case(
