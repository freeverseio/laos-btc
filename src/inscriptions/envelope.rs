...
impl From<RawEnvelope> for ParsedEnvelope {
	fn from(envelope: RawEnvelope) -> Self {
		...
		let unrecognized_even_field =
			fields.keys().any(|tag| tag.first().map(|lsb| lsb % 2 == 0).unwrap_or_default());
...
mod tests {
	...
	fn ignore_unparsable_scripts() {
		...
		assert_eq!(parse(&[Witness::from_slice(&[script_bytes, Vec::new()])]), Vec::new());
	}

	#[test]
	fn no_inscription() {
		assert_eq!(
			parse(&[Witness::from_slice(&[ScriptBuf::new().into_bytes(), Vec::new()])]),
			Vec::new()
		);
	}
	...
	fn duplicate_field() {
		assert_eq!(
			...
			vec![ParsedEnvelope {
				payload: Inscription { duplicate_field: true, ..default() },
				..default()
			}]
		);
	}
	...
	fn no_content_type() {
		assert_eq!(
			...
			vec![ParsedEnvelope {
				payload: Inscription { body: Some(b"foo".to_vec()), ..default() },
				..default()
			}],
		);
	}
	...
	fn no_endif() {
		...
		assert_eq!(parse(&[Witness::from_slice(&[script.into_bytes(), Vec::new()])]), Vec::new(),);
	}
	...
	fn extract_from_second_input() {
		assert_eq!(
			...
			vec![ParsedEnvelope { payload: inscription("foo", [1; 1040]), input: 1, ..default() }]
		);
	}
	...
	fn extract_from_second_envelope() {
		...
		assert_eq!(
			parse(&[Witness::from_slice(&[builder.into_script().into_bytes(), Vec::new()])]),
			vec![
				ParsedEnvelope { payload: inscription("foo", [1; 100]), ..default() },
				ParsedEnvelope { payload: inscription("bar", [1; 100]), offset: 1, ..default() }
			]
		);
	}
	...
	fn inscribe_png() {
		assert_eq!(
			...
			vec![ParsedEnvelope { payload: inscription("image/png", [1; 100]), ..default() }]
		);
	}
	...
	fn chunked_data_is_parsable() {
		...
		assert_eq!(
			parse(&[witness]),
			vec![ParsedEnvelope { payload: inscription("foo", [1; 1040]), ..default() }]
		);
	}
	...
	fn round_trip_with_no_fields() {
		...
		assert_eq!(
			parse(&[witness]),
			vec![ParsedEnvelope { payload: Inscription::default(), ..default() }],
		);
	}
	...
	fn unknown_even_fields() {
		assert_eq!(
			...
			vec![ParsedEnvelope {
				payload: Inscription { unrecognized_even_field: true, ..default() },
				..default()
			}],
		);
	}
	...
	fn pointer_field_is_recognized() {
		assert_eq!(
			...
			vec![ParsedEnvelope {
				payload: Inscription { pointer: Some(vec![1]), ..default() },
				..default()
			}],
		);
	}
	...
	fn incomplete_field() {
		assert_eq!(
			...
			vec![ParsedEnvelope {
				payload: Inscription { incomplete_field: true, ..default() },
				..default()
			}],
		);
	}
	...
	fn metadata_is_parsed_correctly() {
		assert_eq!(
			...
			vec![ParsedEnvelope {
				payload: Inscription { metadata: Some(Vec::new()), ..default() },
				..default()
			}]
		);
	}
	...
	fn pushnum_opcodes_are_parsed_correctly() {
		...
		for &(op, value) in PUSHNUMS {
			...
			assert_eq!(
				parse(&[Witness::from_slice(&[script.into_bytes(), Vec::new()])]),
				vec![ParsedEnvelope {
					payload: Inscription { body: Some(vec![value]), ..default() },
					pushnum: true,
					..default()
				}],
			);
	...
	fn stuttering() {
		...
		assert_eq!(
			parse(&[Witness::from_slice(&[script.into_bytes(), Vec::new()])]),
			vec![ParsedEnvelope { payload: Default::default(), stutter: true, ..default() }],
		);
		...
		assert_eq!(
			parse(&[Witness::from_slice(&[script.into_bytes(), Vec::new()])]),
			vec![ParsedEnvelope { payload: Default::default(), stutter: false, ..default() }],
		);
	}
}
