...
mod tests {
	...
	fn for_extension() {
		...
		assert_eq!(
			...
			("text/plain;charset=utf-8", BrotliEncoderMode::BROTLI_MODE_TEXT)
		);
		assert_regex_match!(
	...
	fn h264_in_mp4_is_allowed() {
		assert!(Media::check_mp4_codec(Path::new("tests/fixtures/h264.mp4")).is_ok(),);
	}

	#[test]
	fn av1_in_mp4_is_rejected() {
		assert!(Media::check_mp4_codec(Path::new("tests/fixtures/av1.mp4")).is_err(),);
	}
