...
impl Default for InscriptionId {
	fn default() -> Self {
		Self { txid: Txid::all_zeros(), index: 0 }
	}
}
...
mod tests {
	...
	fn display() {
		...
		assert_eq!(
			InscriptionId { txid: txid(1), index: 0 }.to_string(),
			"1111111111111111111111111111111111111111111111111111111111111111i0",
		);
		assert_eq!(
			InscriptionId { txid: txid(1), index: 0xFFFFFFFF }.to_string(),
	...
	fn from_str() {
		...
		assert_eq!(
			"1111111111111111111111111111111111111111111111111111111111111111i4294967295"
				.parse::<InscriptionId>()
				.unwrap(),
			InscriptionId { txid: txid(1), index: 0xFFFFFFFF },
		);
		assert_eq!(
			"1111111111111111111111111111111111111111111111111111111111111111i4294967295"
				.parse::<InscriptionId>()
	...
	fn from_str_bad_character() {
		assert_matches!("→".parse::<InscriptionId>(), Err(ParseError::Character('→')),);
	}
