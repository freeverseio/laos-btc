...
impl Settings {
	...
	pub fn merge(options: Options, env: BTreeMap<String, String>) -> Result<Self> {
		...
		let config = if let Some(config_path) = config_path {
			serde_yaml::from_reader(
				File::open(&config_path)
					.context(anyhow!("failed to open config file `{}`", config_path.display()))?,
			)
			.context(anyhow!("failed to deserialize config file `{}`", config_path.display()))?
		} else {
			Settings::default()
		};
		...
		match (&settings.bitcoin_rpc_username, &settings.bitcoin_rpc_password) {
	...
	pub fn from_env(env: BTreeMap<String, String>) -> Result<Self> {
		let get_bool = |key| env.get(key).map(|value| !value.is_empty()).unwrap_or_default();
	...
	pub fn default_data_dir() -> Result<PathBuf> {
		Ok(dirs::data_dir().context("could not get data dir")?.join("ord"))
	}

	pub fn bitcoin_credentials(&self) -> Result<Auth> {
		if let Some((user, pass)) =
			&self.bitcoin_rpc_username.as_ref().zip(self.bitcoin_rpc_password.as_ref())
		{
	...
	pub fn bitcoin_rpc_client(&self, wallet: Option<String>) -> Result<Client> {
		...
		log::trace!("Connecting to Bitcoin Core at {}", self.bitcoin_rpc_url(None));

		if let Auth::CookieFile(cookie_file) = &bitcoin_credentials {
			log::trace!("Using credentials from cookie file at `{}`", cookie_file.display());
	...
	pub fn credentials(&self) -> Option<(&str, &str)> {
		self.server_username.as_deref().zip(self.server_password.as_deref())
	}
...
mod tests {
	...
	fn parse(args: &[&str]) -> Settings {
		let args = iter::once("ord").chain(args.iter().copied()).collect::<Vec<&str>>();
		...
		match Arguments::try_parse_from(args.split_whitespace()) {
			Ok(arguments) => match arguments.subcommand {
				Subcommand::Wallet(wallet) =>
					(Settings::from_options(arguments.options).or_defaults().unwrap(), wallet),
	...
	fn auth_missing_rpc_pass_is_an_error() {
		assert_eq!(
			Settings::merge(
				Options { bitcoin_rpc_username: Some("foo".into()), ..default() },
				Default::default(),
			)
			.unwrap_err()
			.to_string(),
	...
	fn auth_missing_rpc_user_is_an_error() {
		assert_eq!(
			Settings::merge(
				Options { bitcoin_rpc_password: Some("foo".into()), ..default() },
				Default::default(),
			)
			.unwrap_err()
			.to_string(),
	...
	fn cookie_file_overrides_network() {
		assert_eq!(
			parse(&["--cookie-file=/foo/bar", "--chain=signet"]).cookie_file().unwrap(),
			Path::new("/foo/bar")
		);
	}
	...
	fn uses_network_defaults() {
		...
		assert!(settings
			...
			.display()
			.to_string()
			.ends_with(if cfg!(windows) { r"\signet\.cookie" } else { "/signet/.cookie" }));
	}
	...
	fn othernet_cookie_file_path() {
		let cookie_file = parse(&["--chain=signet"]).cookie_file().unwrap().display().to_string();
		...
		let cookie_file = parse(&["--testnet4"]).cookie_file().unwrap().display().to_string();
	...
	fn mainnet_data_dir() {
		...
		assert!(data_dir.ends_with(if cfg!(windows) { r"\ord" } else { "/ord" }), "{data_dir}");
	}
	...
	fn othernet_data_dir() {
		...
		assert!(
			data_dir.ends_with(if cfg!(windows) { r"\ord\signet" } else { "/ord/signet" }),
			"{data_dir}"
		);
	}
	...
	fn network_is_joined_with_data_dir() {
		let data_dir = parse(&["--chain=signet", "--datadir=foo"]).data_dir().display().to_string();
		assert!(
			data_dir.ends_with(if cfg!(windows) { r"foo\signet" } else { "foo/signet" }),
			"{data_dir}"
		);
	}
	...
	fn network_accepts_aliases() {
		...
		check_network_alias("regtest", if cfg!(windows) { r"ord\regtest" } else { "ord/regtest" });
		check_network_alias("signet", if cfg!(windows) { r"ord\signet" } else { "ord/signet" });
		check_network_alias("test", if cfg!(windows) { r"ord\testnet3" } else { "ord/testnet3" });
		check_network_alias(
			"testnet",
			if cfg!(windows) { r"ord\testnet3" } else { "ord/testnet3" },
		);
		check_network_alias(
			"testnet4",
			if cfg!(windows) { r"ord\testnet4" } else { "ord/testnet4" },
		);
	}
	...
	fn uses_wallet_rpc() {
		...
		assert_eq!(settings.bitcoin_rpc_url(Some("foo".into())), "127.0.0.1:8332/wallet/foo");
	}

	#[test]
	fn setting_index_cache_size() {
		assert_eq!(parse(&["--index-cache-size=16000000000",]).index_cache_size(), 16000000000);
	}

	#[test]
	fn setting_commit_interval() {
		let arguments =
			Arguments::try_parse_from(["ord", "--commit-interval", "500", "index", "update"])
				.unwrap();
	...
	fn bitcoin_rpc_and_pass_setting() {
		...
		assert_eq!(
			Settings::merge(
				Options { config: Some(config_path.clone()), ..default() },
				vec![
					("BITCOIN_RPC_USERNAME".into(), "env_user".into()),
					("BITCOIN_RPC_PASSWORD".into(), "env_pass".into()),
				]
	...
	fn merge() {
		...
		let config = Settings { index: Some("config".into()), ..default() };
		...
		pretty_assert_eq!(
			Settings::merge(options.clone(), Default::default()).unwrap().index,
			Some("config".into()),
		);

		pretty_assert_eq!(Settings::merge(options, env.clone()).unwrap().index, Some("env".into()),);
		...
		pretty_assert_eq!(Settings::merge(options, env).unwrap().index, Some("option".into()),);
	}
}
