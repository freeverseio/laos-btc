...
impl FromStr for Outgoing {
	...
	fn from_str(input: &str) -> Result<Self, Self::Err> {
		...
		if re::SAT_NAME.is_match(input) {
			Ok(Outgoing::Sat(input.parse().snafu_context(error::SatParse { input })?))
		} else if re::SATPOINT.is_match(input) {
			Ok(Outgoing::SatPoint(input.parse().snafu_context(error::SatPointParse { input })?))
			...
			Ok(Outgoing::Amount(input.parse().snafu_context(error::AmountParse { input })?))
		} else if let Some(captures) = RUNE.captures(input) {
			let decimal =
				captures[1].parse::<Decimal>().snafu_context(error::RuneAmountParse { input })?;
			let rune = captures[2].parse().snafu_context(error::RuneParse { input })?;
			Ok(Self::Rune { decimal, rune })
		} else {
			Err(SnafuError::OutgoingParse { input: input.to_string() })
		}
	}
}
...
mod tests {
	...
	fn from_str() {
		...
		case(
			"0  : XYZ",
			Outgoing::Rune { rune: "XYZ".parse().unwrap(), decimal: "0".parse().unwrap() },
		);
		...
		case(
			"0.0:XYZ",
			Outgoing::Rune { rune: "XYZ".parse().unwrap(), decimal: "0.0".parse().unwrap() },
		);
		...
		case(
			"1.1:XYZ",
			Outgoing::Rune { rune: "XYZ".parse().unwrap(), decimal: "1.1".parse().unwrap() },
		);

		case(
			"1.1:X.Y.Z",
			Outgoing::Rune { rune: "X.Y.Z".parse().unwrap(), decimal: "1.1".parse().unwrap() },
		);
	}
	...
	fn roundtrip() {
		...
		case("0 btc", Outgoing::Amount("0 btc".parse().unwrap()));
		case("1.20000000 btc", Outgoing::Amount("1.2 btc".parse().unwrap()));

		case(
			"0:XY•Z",
			Outgoing::Rune { rune: "XY•Z".parse().unwrap(), decimal: "0".parse().unwrap() },
		);
	...
	fn serde() {
		...
		case("nvtdijuwxlp", "\"nvtdijuwxlp\"", Outgoing::Sat("nvtdijuwxlp".parse().unwrap()));
		...
		case("3 btc", "\"3 btc\"", Outgoing::Amount(Amount::from_sat(3 * COIN_VALUE)));
