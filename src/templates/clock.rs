	...
	pub(crate) fn new(height: Height) -> Self {
		...
		Self {
			...
			hour: f64::from(min.n() % Epoch::FIRST_POST_SUBSIDY.starting_height().n()) /
				...
				360.0,
			minute: f64::from(min.n() % SUBSIDY_HALVING_INTERVAL) /
				f64::from(SUBSIDY_HALVING_INTERVAL) *
				360.0,
...
mod tests {
	...
	fn final_subsidy_height() {
		pretty_assert_eq!(ClockSvg::new(Height(6929999)).second, 1007.0 / 2016.0 * 360.0);
		pretty_assert_eq!(ClockSvg::new(Height(6929999)).minute, 209_999.0 / 210_000.0 * 360.0);
		pretty_assert_eq!(ClockSvg::new(Height(6929999)).hour, 6929999.0 / 6930000.0 * 360.0);
	}
