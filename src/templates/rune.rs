...
impl RuneHtml {
	fn mint_progress(&self) -> Option<Decimal> {
		...
		Some(Decimal { value: (progress * 10000.0) as u128, scale: 2 })
	}
}
		...
		assert_regex_match!(
			RuneHtml {
				entry: RuneEntry {
					...
					number: 25,
					premine: 123456789,
					spaced_rune: SpacedRune { rune: Rune(u128::MAX), spacers: 1 },
					symbol: Some('@'),
					timestamp: 0,
					turbo: true,
				},
				...
				mintable: true,
				parent: Some(InscriptionId { txid: Txid::all_zeros(), index: 0 }),
			},
