...
use entry::{EtchingEntry, EtchingEntryValue};
use fee_rate::FeeRate;
use index::entry::Entry;
...
impl Wallet {
	pub(crate) fn get_wallet_sat_ranges(&self) -> Result<Vec<(OutPoint, Vec<(u64, u64)>)>> {
		ensure!(self.has_sat_index, "ord index must be built with `--index-sats` to use `--sat`");
	...
	pub(crate) fn find_sat_in_outputs(&self, sat: Sat) -> Result<SatPoint> {
		...
		Err(anyhow!(format!("could not find sat `{sat}` in wallet outputs")))
	}
	...
	pub(crate) fn lock_non_cardinal_outputs(&self) -> Result {
		...
		let locked = self.locked_utxos().keys().cloned().collect::<HashSet<OutPoint>>();
	...
	pub(crate) fn inscription_exists(&self, inscription_id: InscriptionId) -> Result<bool> {
		Ok(!self
			.ord_client
			.get(self.rpc_url.join(&format!("/inscription/{inscription_id}")).unwrap())
			.send()?
			.status()
			.is_client_error())
	}
	...
	pub(crate) fn send_etching(&self, rune: Rune, entry: &EtchingEntry) -> Result<batch::Output> {
		...
		Ok(batch::Output { reveal_broadcast: true, ..entry.output.clone() })
	}
	...
	pub(crate) fn initialize(
		...
		let derivation_path = DerivationPath::master()
			...
			.child(ChildNumber::Hardened { index: u32::from(network != Network::Bitcoin) })
		...
		for change in [false, true] {
			let secret_key = DescriptorSecretKey::XPrv(DescriptorXKey {
				...
				derivation_path: DerivationPath::master()
					.child(ChildNumber::Normal { index: change.into() }),
		...
		match settings.bitcoin_rpc_client(Some(name.clone()))?.call::<serde_json::Value>(
			"importdescriptors",
	...
	fn format_bitcoin_core_version(version: usize) -> String {
		format!("{}.{}.{}", version / 10000, version % 10000 / 100, version % 100)
	}

	pub(crate) fn open_database(wallet_name: &String, settings: &Settings) -> Result<Database> {
		let path = settings.data_dir().join("wallets").join(format!("{wallet_name}.redb"));
		...
		let database = match Database::builder().set_repair_callback(repair_callback).open(&path) {
			Ok(database) => {
				{
				...
				if error.kind() == io::ErrorKind::NotFound =>
			{
				...
				tx.open_table(STATISTICS)?.insert(&Statistic::Schema.key(), &SCHEMA_VERSION)?;
	...
	pub(crate) fn save_etching(
		...
		wtx.open_table(RUNE_TO_ETCHING)?.insert(
			rune.0,
			EtchingEntry { commit: commit.clone(), reveal: reveal.clone(), output }.store(),
		)?;
	...
	fn send_raw_transaction<R: bitcoincore_rpc::RawTx>(
		...
		Ok(self.bitcoin_client().call("sendrawtransaction", &arguments)?)
	}
	...
	pub fn create_unsigned_send_amount_transaction(
		...
		let unfunded_transaction = Transaction {
			...
			input: Vec::new(),
			output: vec![TxOut { script_pubkey: destination.script_pubkey(), value: amount }],
		};
	...
	pub fn create_unsigned_send_or_burn_runes_transaction(
		...
		let input_rune_balance =
			input_rune_balances.get(&spaced_rune.rune).cloned().unwrap_or_default();

		let needs_runes_change_output =
			input_rune_balance > amount || input_rune_balances.len() > 1;
		...
		let unfunded_transaction = if let Some(destination) = destination {
			runestone = Runestone { edicts: vec![Edict { amount, id, output: 2 }], ..default() };
			...
			Transaction {
				...
				output: if needs_runes_change_output {
					vec![
						TxOut { script_pubkey: runestone.encipher(), value: Amount::from_sat(0) },
						TxOut {
							script_pubkey: self.get_change_address()?.script_pubkey(),
							value: postage,
						},
						TxOut { script_pubkey: destination.script_pubkey(), value: postage },
					]
				} else {
					vec![TxOut { script_pubkey: destination.script_pubkey(), value: postage }]
				},
			}
		} else {
			runestone = Runestone { edicts: vec![Edict { amount, id, output: 0 }], ..default() };
			...
			Transaction {
				...
				output: if needs_runes_change_output {
					...
					vec![TxOut { script_pubkey: runestone.encipher(), value: Amount::from_sat(0) }]
				},
			}
		};
