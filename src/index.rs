...
use self::{
	...
	lot::Lot,
	reorg::Reorg,
	updater::{Brc721TokenId, RegisterCollectionValue, Updater},
...
define_table! { BRC721_TOKENID_TO_BRC721_COLLECTION_ID, Brc721TokenId, Brc721CollectionIdValue}
...
impl<T> BitcoinCoreRpcResultExt<T> for Result<T, bitcoincore_rpc::Error> {
	fn into_option(self) -> Result<Option<T>> {
		match self {
			...
			Err(bitcoincore_rpc::Error::JsonRpc(bitcoincore_rpc::jsonrpc::error::Error::Rpc(
				bitcoincore_rpc::jsonrpc::error::RpcError { code: -5, message, .. },
...
impl Index {
	...
	pub fn open_with_event_sender(
		...
		let durability =
			if cfg!(test) { redb::Durability::None } else { redb::Durability::Immediate };
			...
			.open(&path)
		{
			...
			{
				let database =
					Database::builder().set_cache_size(index_cache_size).create(&path)?;
				...
				if settings.index_runes_raw() && settings.chain() == Chain::Mainnet {
					...
					tx.open_table(RUNE_TO_RUNE_ID)?.insert(rune.store(), id.store())?;
	...
	pub fn info(&self) -> Result<Info> {
		...
		let total_bytes = tables.values().map(|table_info| table_info.total_bytes).sum();
		...
		let info = {
			...
			Info {
				...
				transactions: rtx
					...
					.range(0..)?
					.flat_map(|result| {
						result.map(|(starting_block_count, starting_timestamp)| TransactionInfo {
	...
	pub fn export(&self, filename: &String, include_addresses: bool) -> Result {
		...
		for result in rtx.open_table(SEQUENCE_NUMBER_TO_INSCRIPTION_ENTRY)?.iter()? {
			let entry = result?;
			let sequence_number = entry.0.value();
			let entry = InscriptionEntry::load(entry.1.value());
			let satpoint =
				SatPoint::load(*sequence_number_to_satpoint.get(sequence_number)?.unwrap().value());

			write!(writer, "{}\t{}\t{}", entry.inscription_number, entry.id, satpoint)?;
	...
	pub(crate) fn is_statistic_set(
		...
		Ok(statistics.get(&statistic.key())?.map(|guard| guard.value()).unwrap_or_default() != 0)
	}
	...
	pub(crate) fn inscription_number(&self, inscription_id: InscriptionId) -> i32 {
		self.get_inscription_entry(inscription_id).unwrap().unwrap().inscription_number
	}
	...
	pub fn blocks(&self, take: usize) -> Result<Vec<(u32, BlockHash)>> {
		...
		for next in height_to_block_header.range(0..block_count)?.rev().take(take) {
			let next = next?;
	...
	pub fn rune(&self, rune: Rune) -> Result<Option<(RuneId, RuneEntry, Option<InscriptionId>)>> {
		...
		let Some(id) = rtx.open_table(RUNE_TO_RUNE_ID)?.get(rune.0)?.map(|guard| guard.value())
		else {
			return Ok(None);
		};

		let entry =
			RuneEntry::load(rtx.open_table(RUNE_ID_TO_RUNE_ENTRY)?.get(id)?.unwrap().value());

		let parent = InscriptionId { txid: entry.etching, index: 0 };
	...
	pub fn runes(&self) -> Result<Vec<(RuneId, RuneEntry)>> {
		...
		for result in self.database.begin_read()?.open_table(RUNE_ID_TO_RUNE_ENTRY)?.iter()? {
			let (id, entry) = result?;
	...
	pub fn decode_rune_balance(buffer: &[u8]) -> Result<((RuneId, u128), usize)> {
		...
		len += tx_len;
		let id = RuneId { block: block.try_into()?, tx: tx.try_into()? };
	...
	pub fn get_rune_balances_for_output(
		...
		while i < balances_buffer.len() {
			...
			balances.insert(
				entry.spaced_rune,
				Pile { amount, divisibility: entry.divisibility, symbol: entry.symbol },
			);
		}
	...
	pub fn get_rune_balance_map(&self) -> Result<BTreeMap<SpacedRune, BTreeMap<OutPoint, Pile>>> {
		...
		for (outpoint, balances) in outpoint_balances {
			for (rune_id, amount) in balances {
				*rune_balances_by_id.entry(rune_id).or_default().entry(outpoint).or_default() +=
					amount;
			}
		}
		...
		for (rune_id, balances) in rune_balances_by_id {
			let RuneEntry { divisibility, spaced_rune, symbol, .. } =
				RuneEntry::load(rune_id_to_rune_entry.get(&rune_id.store())?.unwrap().value());

			rune_balances.insert(
				spaced_rune,
				balances
					.into_iter()
					.map(|(outpoint, amount)| (outpoint, Pile { amount, divisibility, symbol }))
					.collect(),
			);
		}
	...
	pub fn get_rune_balances(&self) -> Result<Vec<(OutPoint, Vec<(RuneId, u128)>)>> {
		...
		for entry in self.database.begin_read()?.open_table(OUTPOINT_TO_RUNE_BALANCES)?.iter()? {
	...
	pub(crate) fn get_children_by_inscription_id(
		...
		self.get_children_by_sequence_number_paginated(sequence_number, usize::MAX, 0)
	...
	pub(crate) fn get_parents_by_inscription_id(
		...
		let sequence_number_to_inscription_entry =
			rtx.open_table(SEQUENCE_NUMBER_TO_INSCRIPTION_ENTRY).unwrap();
	...
	pub fn get_inscription_id_by_sat_indexed(
		...
		if inscription_index < 0 {
			...
			sat_to_sequence_number.get(&sat.n())?.nth(inscription_index.abs_diff(0))
		}
		.map(|result| {
			result
				.and_then(|sequence_number| {
	...
	pub fn inscription_count(&self, txid: Txid) -> Result<u32> {
		...
		let end = InscriptionId { index: u32::MAX, txid };
	...
	pub fn get_inscriptions_for_output(
		...
		Ok(Some(inscriptions.iter().map(|(_satpoint, inscription_id)| *inscription_id).collect()))
	}
	...
	pub fn get_inscriptions_for_outputs(
		...
		for outpoint in outpoints {
			let Some(inscriptions) = self.get_inscriptions_on_output_with_satpoints(*outpoint)?
			else {
				return Ok(None);
			};

			result.extend(inscriptions.iter().map(|(_satpoint, inscription_id)| *inscription_id));
		}
	...
	pub fn is_output_spent(&self, outpoint: OutPoint) -> Result<bool> {
		Ok(outpoint != OutPoint::null() &&
			...
			if self.have_full_utxo_index() {
				...
				self.client.get_tx_out(&outpoint.txid, outpoint.vout, Some(true))?.is_none()
			})
	}
	...
	pub fn is_output_in_active_chain(&self, outpoint: OutPoint) -> Result<bool> {
		...
		let Some(info) =
			self.client.get_raw_transaction_info(&outpoint.txid, None).into_option()?
		else {
			return Ok(false);
		};
	...
	pub fn get_inscriptions_in_block(&self, block_height: u32) -> Result<Vec<InscriptionId>> {
		...
		let Some(newest_sequence_number) =
			height_to_last_sequence_number.get(&block_height)?.map(|ag| ag.value())
		else {
			return Ok(Vec::new());
		};
		...
		(oldest_sequence_number..newest_sequence_number)
			.map(|num| match sequence_number_to_inscription_entry.get(&num) {
				...
				Ok(None) => Err(anyhow!("could not find inscription for inscription number {num}")),
	...
	pub fn get_runes_in_block(&self, block_height: u64) -> Result<Vec<SpacedRune>> {
		...
		let min_id = RuneId { block: block_height, tx: 0 };

		let max_id = RuneId { block: block_height, tx: u32::MAX };
	...
	pub fn get_highest_paying_inscriptions_in_block(
		...
		Ok((
			inscription_to_fee.iter().map(|(id, _)| *id).rev().take(n).collect(),
			inscription_ids.len(),
		))
	}
	...
	pub(crate) fn inscription_info(
		...
		let entry = InscriptionEntry::load(
			sequence_number_to_inscription_entry.get(&sequence_number)?.unwrap().value(),
		);
		...
		let output =
			if satpoint.outpoint == unbound_outpoint() || satpoint.outpoint == OutPoint::null() {
				...
				transaction.output.into_iter().nth(satpoint.outpoint.vout.try_into().unwrap())
			};

		let previous = if let Some(n) = sequence_number.checked_sub(1) {
			Some(
				InscriptionEntry::load(
					sequence_number_to_inscription_entry.get(n)?.unwrap().value(),
				)
				.id,
			)
		} else {
		...
		let all_children =
			rtx.open_multimap_table(SEQUENCE_NUMBER_TO_CHILDREN)?.get(sequence_number)?;
		...
		let rune = if let Some(rune_id) =
			rtx.open_table(SEQUENCE_NUMBER_TO_RUNE_ID)?.get(sequence_number)?
		{
			let rune_id_to_rune_entry = rtx.open_table(RUNE_ID_TO_RUNE_ENTRY)?;
		...
		let parents = entry
			...
			.take(4)
			.map(|parent| {
				Ok(InscriptionEntry::load(
					sequence_number_to_inscription_entry.get(parent)?.unwrap().value(),
				)
				.id)
			})
			.collect::<Result<Vec<InscriptionId>>>()?;
	...
	fn assert_inscription_location(
		...
		assert_eq!(
			SatPoint::load(
				*sequence_number_to_satpoint.get(sequence_number).unwrap().unwrap().value()
			),
			satpoint,
		);

		let utxo_entry = outpoint_to_utxo_entry.get(&satpoint.outpoint.store()).unwrap().unwrap();
	...
	pub fn get_address_info(&self, address: &Address) -> Result<Vec<OutPoint>> {
		self.database
			...
			.map(|result| {
				result.map_err(|err| anyhow!(err)).map(|value| OutPoint::load(value.value()))
			})
			.collect()
	}
		...
		for output in outputs {
			...
			for (spaced_rune, pile) in rune_balances {
				runes
					...
					.or_insert((
						Decimal { value: pile.amount, scale: pile.divisibility },
						pile.symbol,
					));
			}
		}
	...
	pub(crate) fn get_sat_balances_for_outputs(&self, outputs: &Vec<OutPoint>) -> Result<u64> {
		let outpoint_to_utxo_entry =
			self.database.begin_read()?.open_table(OUTPOINT_TO_UTXO_ENTRY)?;
...
mod tests {
	use super::*;
	use crate::index::testing::Context;
	...
	fn inscriptions_below_first_inscription_height_are_skipped() {
		...
		let template =
			TransactionTemplate { inputs: &[(1, 0, 0, inscription.to_witness())], ..default() };

		{
			...
			assert_eq!(
				context.index.get_inscription_satpoint_by_id(inscription_id).unwrap(),
				Some(SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 0 })
			);
		}

		{
			...
			assert_eq!(context.index.get_inscription_satpoint_by_id(inscription_id).unwrap(), None,);
		}
	}
	...
	fn list_merge_ranges_are_tracked_correctly() {
		...
		assert_eq!(
			context.index.list(OutPoint::new(txid, 0)).unwrap().unwrap(),
			&[(50 * COIN_VALUE, 100 * COIN_VALUE), (100 * COIN_VALUE, 150 * COIN_VALUE)],
		);
	}
	...
	fn list_fee_paying_transaction_range() {
		...
		assert_eq!(
			context.index.list(OutPoint::new(coinbase_txid, 0)).unwrap().unwrap(),
	...
	fn list_two_fee_paying_transaction_range() {
		...
		context.mine_blocks(2);
		let first_fee_paying_tx =
			TransactionTemplate { inputs: &[(1, 0, 0, Default::default())], fee: 10, ..default() };
		let second_fee_paying_tx =
			TransactionTemplate { inputs: &[(2, 0, 0, Default::default())], fee: 10, ..default() };
		...
		assert_eq!(
			context.index.list(OutPoint::new(coinbase_txid, 0)).unwrap().unwrap(),
			&[(15000000000, 20000000000), (9999999990, 10000000000), (14999999990, 15000000000)],
		);
	}
	...
	fn list_null_output() {
		...
		assert_eq!(context.index.list(OutPoint::new(txid, 0)).unwrap().unwrap(), &[],);
	}
	...
	fn list_null_input() {
		...
		let no_value_input =
			TransactionTemplate { inputs: &[(2, 1, 0, Default::default())], fee: 0, ..default() };
	...
	fn find_first_sat_of_second_block() {
		...
		assert_eq!(
			...
			SatPoint { outpoint: OutPoint { txid: tx.compute_txid(), vout: 0 }, offset: 0 }
		)
	}
	...
	fn find_first_sat_spent_in_second_block() {
		...
		assert_eq!(
			context.index.find(Sat(50 * COIN_VALUE)).unwrap().unwrap(),
			SatPoint { outpoint: OutPoint::new(spend_txid, 0), offset: 0 }
		)
	}
	...
	fn inscriptions_are_tracked_correctly() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 0 },
				Some(50 * COIN_VALUE),
			);
		}
	}
	...
	fn inscriptions_without_sats_are_unbound() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: unbound_outpoint(), offset: 0 },
				None,
			);
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: unbound_outpoint(), offset: 1 },
				None,
			);
		}
	}
	...
	fn unaligned_inscriptions_are_tracked_correctly() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint {
					outpoint: OutPoint { txid: send_txid, vout: 0 },
	...
	fn merged_inscriptions_are_tracked_correctly() {
		for context in Context::configurations() {
			...
			let second_inscription_id = InscriptionId { txid: second_txid, index: 0 };
			...
			context.index.assert_inscription_location(
				...
				SatPoint { outpoint: OutPoint { txid: merged_txid, vout: 0 }, offset: 0 },
				Some(50 * COIN_VALUE),
			);
			...
			context.index.assert_inscription_location(
				...
				SatPoint {
					outpoint: OutPoint { txid: merged_txid, vout: 0 },
					offset: 50 * COIN_VALUE,
				},
				Some(100 * COIN_VALUE),
			);
	...
	fn inscriptions_that_are_sent_to_second_output_are_are_tracked_correctly() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint { txid: send_txid, vout: 1 }, offset: 0 },
				Some(50 * COIN_VALUE),
			);
		}
	}
	...
	fn one_input_fee_spent_inscriptions_are_tracked_correctly() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint {
					outpoint: OutPoint { txid: coinbase_tx, vout: 0 },
					offset: 50 * COIN_VALUE,
				},
				Some(50 * COIN_VALUE),
			);
	...
	fn lost_inscriptions() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint::null(), offset: 0 },
				Some(50 * COIN_VALUE),
			);
		}
	}
	...
	fn multiple_inscriptions_can_be_lost() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				second_inscription_id,
				SatPoint { outpoint: OutPoint::null(), offset: 50 * COIN_VALUE },
				Some(150 * COIN_VALUE),
			);
		}
	}
	...
	fn lost_sats_are_tracked_correctly() {
		...
		context.mine_blocks_with_subsidy(1, 0);
		assert_eq!(context.index.statistic(Statistic::LostSats), 50 * COIN_VALUE);

		context.mine_blocks_with_subsidy(1, 0);
		assert_eq!(context.index.statistic(Statistic::LostSats), 100 * COIN_VALUE);
	...
	fn lost_sat_ranges_are_tracked_correctly() {
		...
		let null_ranges = || context.index.list(OutPoint::null()).unwrap().unwrap_or_default();
		...
		assert_eq!(
			null_ranges(),
			[(100 * COIN_VALUE, 150 * COIN_VALUE), (150 * COIN_VALUE, 200 * COIN_VALUE)]
		);
	...
	fn lost_inscriptions_get_lost_satpoints() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint::null(), offset: 75 * COIN_VALUE },
				Some(100 * COIN_VALUE),
			);
		}
	}
	...
	fn inscription_skips_zero_value_first_output_of_inscribe_transaction() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint { txid, vout: 1 }, offset: 0 },
				Some(50 * COIN_VALUE),
			);
		}
	}
	...
	fn lost_rare_sats_are_tracked() {
		...
		assert_eq!(
			context.index.rare_sat_satpoint(Sat(50 * COIN_VALUE)).unwrap().unwrap(),
			SatPoint { outpoint: OutPoint::null(), offset: 0 },
		);

		assert_eq!(
			context.index.rare_sat_satpoint(Sat(100 * COIN_VALUE)).unwrap().unwrap(),
			SatPoint { outpoint: OutPoint::null(), offset: 50 * COIN_VALUE },
		);
	}
	...
	fn old_schema_gives_correct_error() {
		let tempdir = {
			...
			wtx.open_table(STATISTIC_TO_COUNT)
				.unwrap()
	...
	fn inscriptions_on_output() {
		for context in Context::configurations() {
			...
			assert_eq!(
				context
					.index
					.get_inscriptions_for_output(OutPoint { txid: send_id, vout: 0 })
					.unwrap()
					.unwrap_or_default(),
				[inscription_id]
			);
	...
	fn inscriptions_on_same_sat_after_the_first_are_not_unbound() {
		for context in Context::configurations() {
			...
			let inscription_id = InscriptionId { txid: first, index: 0 };

			assert_eq!(
				context
					.index
					.get_inscriptions_for_output(OutPoint { txid: first, vout: 0 })
					.unwrap()
					.unwrap_or_default(),
				[inscription_id]
			);
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint { txid: first, vout: 0 }, offset: 0 },
				Some(50 * COIN_VALUE),
			);
			...
			let inscription_id = InscriptionId { txid: second, index: 0 };

			context.mine_blocks(1);

			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint { txid: second, vout: 0 }, offset: 0 },
				Some(50 * COIN_VALUE),
			);

			assert!(context
				.index
				.get_inscription_by_id(InscriptionId { txid: second, index: 0 })
				.unwrap()
				.is_some());
	...
	fn unrecognized_even_field_inscriptions_are_cursed_and_unbound() {
		for context in Context::configurations() {
			...
			let witness =
				envelope(&[b"ord", &[1], b"text/plain;charset=utf-8", &[2], b"bar", &[4], b"ord"]);

			let txid = context
				.core
				.broadcast_tx(TransactionTemplate { inputs: &[(1, 0, 0, witness)], ..default() });
	...
	fn inscriptions_are_uncursed_after_jubilee() {
		for context in Context::configurations() {
			...
			let txid = context
				.core
				.broadcast_tx(TransactionTemplate { inputs: &[(2, 0, 0, witness)], ..default() });
	...
	fn multiple_inscriptions_in_same_tx_all_but_first_input_are_cursed() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				second,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 50 * COIN_VALUE },
				Some(100 * COIN_VALUE),
			);

			context.index.assert_inscription_location(
				third,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 100 * COIN_VALUE },
				Some(150 * COIN_VALUE),
			);
	...
	fn multiple_inscriptions_different_inputs_and_same_inputs() {
		for context in Context::configurations() {
			...
			assert_eq!(
				context.index.get_inscription_id_by_inscription_number(-3).unwrap().unwrap(),
				fourth
			);
	...
	fn inscription_fee_distributed_evenly() {
		for context in Context::configurations() {
			...
			assert_eq!(context.index.get_inscription_entry(first).unwrap().unwrap().fee, 11);

			assert_eq!(context.index.get_inscription_entry(second).unwrap().unwrap().fee, 11);
		}
	}
	...
	fn reinscription_on_cursed_inscription_is_not_cursed() {
		for context in Context::configurations() {
			...
			let cursed = InscriptionId { txid: cursed_txid, index: 1 };

			context.mine_blocks(1);

			context.index.assert_inscription_location(
				cursed,
				SatPoint { outpoint: OutPoint { txid: cursed_txid, vout: 1 }, offset: 0 },
				Some(100 * COIN_VALUE),
			);
			...
			let txid = context
				.core
				.broadcast_tx(TransactionTemplate { inputs: &[(3, 1, 1, witness)], ..default() });
	...
	fn second_reinscription_on_cursed_inscription_is_cursed() {
		for context in Context::configurations() {
			...
			let txid = context
				.core
				.broadcast_tx(TransactionTemplate { inputs: &[(4, 1, 0, witness)], ..default() });
			...
			assert_eq!(context.index.inscription_number(second_reinscription_on_cursed), -2);

			assert_eq!(
				vec![cursed, reinscription_on_cursed, second_reinscription_on_cursed],
				context
					.index
	...
	fn reinscriptions_on_output_correctly_ordered_and_transferred() {
		for context in Context::configurations() {
			...
			let txid = context.core.broadcast_tx(TransactionTemplate {
				inputs: &[(2, 1, 0, inscription("text/plain;charset=utf-8", "hello").to_witness())],
				..default()
			});
			...
			context.mine_blocks(1);
			let txid = context.core.broadcast_tx(TransactionTemplate {
				inputs: &[(3, 1, 0, inscription("text/plain;charset=utf-8", "hello").to_witness())],
				..default()
			});
			...
			let location = SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 0 };
	...
	fn reinscriptions_are_ordered_correctly_for_many_outpoints() {
		for context in Context::configurations() {
			...
			let location = SatPoint { outpoint: OutPoint { txid: final_txid, vout: 0 }, offset: 0 };
	...
	fn recover_from_reorg() {
		for mut context in Context::configurations() {
			...
			let txid = context.core.broadcast_tx(TransactionTemplate {
				inputs: &[(2, 0, 0, inscription("text/plain;charset=utf-8", "hello").to_witness())],
				..default()
			});
			let second_id = InscriptionId { txid, index: 0 };
			let second_location = SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 0 };
	...
	fn inscription_with_parent_tag_without_parent_has_no_parent_entry() {
		for context in Context::configurations() {
			...
			let parent_inscription_id = InscriptionId { txid: parent_txid, index: 0 };
	...
	fn inscription_with_parent_tag_and_parent_has_parent_entry() {
		for context in Context::configurations() {
			...
			assert_eq!(
				context.index.get_children_by_inscription_id(parent_inscription_id).unwrap(),
				vec![inscription_id]
			);
		}
	}
	...
	fn inscription_with_two_parent_tags_and_parents_has_parent_entries() {
		for context in Context::configurations() {
			...
			let parent_inscription_id_a = InscriptionId { txid: parent_txid_a, index: 0 };
			let parent_inscription_id_b = InscriptionId { txid: parent_txid_b, index: 0 };

			let multi_parent_inscription = Inscription {
				content_type: Some("text/plain".into()),
				body: Some("hello".into()),
				parents: vec![parent_inscription_id_a.value(), parent_inscription_id_b.value()],
				..default()
			};
			...
			assert_eq!(
				context.index.get_children_by_inscription_id(parent_inscription_id_a).unwrap(),
				vec![inscription_id]
			);
			assert_eq!(
				context.index.get_children_by_inscription_id(parent_inscription_id_b).unwrap(),
				vec![inscription_id]
			);
		}
	}
	...
	fn inscription_with_distinct_parent_tag_encodings_for_same_parent_has_singular_parent_entry() {
		for context in Context::configurations() {
			...
			let trailing_zero_inscription_id: Vec<u8> =
				parent_inscription_id.value().into_iter().chain(vec![0, 0, 0, 0]).collect();
	...
	fn inscription_with_three_parent_tags_and_two_parents_has_two_parent_entries() {
		for context in Context::configurations() {
			...
			let parent_inscription_id_a = InscriptionId { txid: parent_txid_a, index: 0 };
			let parent_inscription_id_b = InscriptionId { txid: parent_txid_b, index: 0 };
			let parent_inscription_id_c = InscriptionId { txid: parent_txid_c, index: 0 };
			...
			assert_eq!(
				context.index.get_children_by_inscription_id(parent_inscription_id_c).unwrap(),
				vec![inscription_id]
			);
		}
	}
	...
	fn inscription_with_valid_and_malformed_parent_tags_only_lists_valid_entries() {
		for context in Context::configurations() {
			...
			let malformed_inscription_id_b =
				parent_inscription_id_b.value().into_iter().chain(iter::once(0)).collect();
	...
	fn inscription_with_pointer() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 100 },
	...
	fn inscription_with_pointer_to_parent_is_cursed_reinscription() {
		for context in Context::configurations() {
			...
			assert_eq!(
				context.index.get_parents_by_inscription_id(child_inscription_id),
				vec![parent_inscription_id]
			);
	...
	fn inscriptions_in_same_input_with_pointers_to_same_output() {
		for context in Context::configurations() {
			...
			let builder = Inscription { pointer: Some(100u64.to_le_bytes().to_vec()), ..default() }
				.append_reveal_script_to_builder(builder);

			let builder =
				Inscription { pointer: Some(300_000u64.to_le_bytes().to_vec()), ..default() }
					.append_reveal_script_to_builder(builder);

			let builder =
				Inscription { pointer: Some(1_000_000u64.to_le_bytes().to_vec()), ..default() }
					.append_reveal_script_to_builder(builder);
			...
			context.index.assert_inscription_location(
				second,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 300_000 },
				Some(50 * COIN_VALUE + 300_000),
			);

			context.index.assert_inscription_location(
				third,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 1_000_000 },
	...
	fn inscriptions_in_same_input_with_pointers_to_different_outputs() {
		for context in Context::configurations() {
			...
			let builder =
				Inscription { pointer: Some(100_111u64.to_le_bytes().to_vec()), ..default() }
					.append_reveal_script_to_builder(builder);

			let builder =
				Inscription { pointer: Some(299_999u64.to_le_bytes().to_vec()), ..default() }
					.append_reveal_script_to_builder(builder);
			...
			context.index.assert_inscription_location(
				second,
				SatPoint { outpoint: OutPoint { txid, vout: 1 }, offset: 111 },
				Some(50 * COIN_VALUE + 100_111),
			);

			context.index.assert_inscription_location(
				third,
				SatPoint { outpoint: OutPoint { txid, vout: 2 }, offset: 99_999 },
	...
	fn inscriptions_in_different_inputs_with_pointers_to_different_outputs() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				...
				SatPoint { outpoint: OutPoint { txid, vout: 2 }, offset: 0 },
				Some(150 * COIN_VALUE),
			);
	...
	fn inscriptions_in_different_inputs_with_pointers_to_same_output() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				second_inscription_id,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 1 },
				Some(50 * COIN_VALUE + 1),
			);

			context.index.assert_inscription_location(
				third_inscription_id,
				SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 2 },
	...
	fn inscriptions_with_pointers_to_same_sat_one_becomes_cursed_reinscriptions() {
		for context in Context::configurations() {
			...
			assert_eq!(context.index.inscription_number(cursed_reinscription_id), -1);
		}
	}
	...
	fn inscribe_into_fee() {
		for context in Context::configurations() {
			...
			context.index.assert_inscription_location(
				inscription_id,
				SatPoint {
					outpoint: OutPoint { txid: blocks[0].txdata[0].compute_txid(), vout: 0 },
					offset: 50 * COIN_VALUE,
				},
				Some(50 * COIN_VALUE),
			);
	...
	fn pre_jubilee_first_reinscription_after_cursed_inscription_is_blessed() {
		for context in Context::configurations() {
			...
			let entry = context.index.get_inscription_entry(inscription_id).unwrap().unwrap();

			assert!(Charm::charms(entry.charms).iter().any(|charm| *charm == Charm::Cursed));

			assert!(!Charm::charms(entry.charms).iter().any(|charm| *charm == Charm::Vindicated));
	...
	fn is_output_spent() {
		...
		assert!(!context
			.index
			.is_output_spent(OutPoint { txid: context.core.tx(1, 0).compute_txid(), vout: 0 })
			.unwrap());
	...
	fn is_output_in_active_chain() {
		...
		assert!(context.index.is_output_in_active_chain(OutPoint::null()).unwrap());
		...
		assert!(!context
			.index
			.is_output_in_active_chain(OutPoint { txid: Txid::all_zeros(), vout: 0 })
			.unwrap());
	}

	#[test]
	fn output_addresses_are_updated() {
		let context = Context::builder().arg("--index-addresses").arg("--index-sats").build();
		...
		let first_address_second_output = OutPoint { txid: transaction.compute_txid(), vout: 1 };

		assert_eq!(
			context.index.get_address_info(&first_address).unwrap(),
			[OutPoint { txid: transaction.compute_txid(), vout: 0 }, first_address_second_output]
		);
		...
		assert_eq!(
			...
			[OutPoint { txid: transaction.compute_txid(), vout: 0 }]
		);
	}
		...
		assert_eq!(
			...
			Event::InscriptionCreated {
				...
				location: Some(SatPoint {
					outpoint: OutPoint { txid: create_txid, vout: 0 },
					offset: 0
				}),
				sequence_number: 0,
				block_height: 2,
		...
		assert_eq!(
			...
			Event::InscriptionTransferred {
				...
				new_location: SatPoint {
					outpoint: OutPoint { txid: transfer_txid, vout: 0 },
					offset: 0
				},
	...
	fn rune_event_sender_channel() {
		...
		let (event_sender, mut event_receiver) = tokio::sync::mpsc::channel(1024);
		let context = Context::builder().arg("--index-runes").event_sender(event_sender).build();
		...
		context.assert_runes(
			[(
				...
				RuneEntry {
					block: id.block,
					etching: txid0,
					spaced_rune: SpacedRune { rune: Rune(RUNE), spacers: 0 },
					timestamp: id.block,
					mints: 0,
					terms: Some(Terms { amount: Some(1000), cap: Some(100), ..default() }),
					..default()
		...
		assert_eq!(
			...
			Event::RuneEtched { block_height: 8, txid: txid0, rune_id: id }
		);

		let txid1 = context.core.broadcast_tx(TransactionTemplate {
			inputs: &[(2, 0, 0, Witness::new())],
			op_return: Some(Runestone { mint: Some(id), ..default() }.encipher()),
			..default()
		});
		...
		context.assert_runes(
			...
			[(OutPoint { txid: txid1, vout: 0 }, vec![(id, 1000)])],
		);

		assert_eq!(
			event_receiver.blocking_recv().unwrap(),
			Event::RuneMinted { block_height: 9, txid: txid1, rune_id: id, amount: 1000 }
		);
		...
		let txid2 = context.core.broadcast_tx(TransactionTemplate {
			...
			op_return: Some(
				Runestone {
					edicts: vec![Edict { id, amount: 1000, output: 0 }],
					..Default::default()
				}
				.encipher(),
			),
		...
		context.assert_runes(
			[(
				...
				RuneEntry {
					block: 8,
					etching: txid0,
					spaced_rune: SpacedRune { rune: Rune(RUNE), ..default() },
					terms: Some(Terms { amount: Some(1000), cap: Some(100), ..Default::default() }),
					timestamp: 8,
					mints: 1,
					..Default::default()
				},
			)],
			[(OutPoint { txid: txid2, vout: 0 }, vec![(id, 1000)])],
		);
		...
		pretty_assert_eq!(
			...
			Event::RuneTransferred {
				...
				rune_id: id,
				amount: 1000,
				outpoint: OutPoint { txid: txid2, vout: 0 },
			}
		);
		...
		let txid3 = context.core.broadcast_tx(TransactionTemplate {
			...
			op_return: Some(
				Runestone {
					edicts: vec![Edict { id, amount: 111, output: 0 }],
					..Default::default()
				}
				.encipher(),
			),
		...
		context.assert_runes(
			...
			[(OutPoint { txid: txid3, vout: 1 }, vec![(id, 889)])],
		);
		...
		pretty_assert_eq!(
			event_receiver.blocking_recv().unwrap(),
			Event::RuneBurned { block_height: 11, txid: txid3, amount: 111, rune_id: id }
		);
	}
