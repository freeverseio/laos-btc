...
enum Error {
	DustOutput { value: Amount, threshold: Amount, output: usize },
	DustPostage { value: Amount, threshold: Amount },
	NoOutputs,
	RunestoneSize { size: usize },
	Shortfall { rune: SpacedRune, have: Pile, need: Pile },
	ZeroValue { output: usize, rune: SpacedRune },
}

impl Display for Error {
	fn fmt(&self, f: &mut Formatter) -> fmt::Result {
		match self {
			Self::DustOutput { value, threshold, output } => {
				write!(f, "output {output} value {value} below dust threshold {threshold}")
			},
...
impl Split {
	pub(crate) fn run(self, wallet: Wallet) -> SubcommandResult {
		...
		let unsigned_transaction =
			fund_raw_transaction(wallet.bitcoin_client(), self.fee_rate, &unfunded_transaction)?;
	...
	fn build_transaction(
		...
		if postage < change_script_pubkey.minimal_non_dust() {
			return Err(Error::DustPostage { value: postage, threshold: change_dust_threshold });
		}
		...
		let runestone = Runestone { edicts, ..default() };
		...
		output.push(TxOut { script_pubkey: runestone_script_pubkey, value: Amount::from_sat(0) });

		if need_rune_change_output {
			output.push(TxOut { script_pubkey: change_script_pubkey, value: postage });
		}
		...
		for (i, split_output) in splits.outputs.iter().enumerate() {
			...
			if value < threshold {
				return Err(Error::DustOutput { output: i, threshold, value });
			}
		...
		assert_eq!(Runestone::decipher(&tx), Some(Artifact::Runestone(runestone)),);
...
mod tests {
	...
	fn splits_must_have_at_least_one_output() {
		assert_eq!(
			Split::build_transaction(
				...
				&change(0),
				None,
				&Splitfile { outputs: Vec::new(), rune_info: BTreeMap::new() },
			)
			.unwrap_err(),
			Error::NoOutputs,
		);
	...
	fn postage_may_not_be_dust() {
		assert_eq!(
			...
			.unwrap_err(),
			Error::DustPostage { value: Amount::from_sat(100), threshold: Amount::from_sat(294) },
		);
	}
	...
	fn output_rune_value_may_not_be_zero() {
		assert_eq!(
				...
				&Splitfile {
					...
					rune_info: [(
						...
						RuneInfo {
							...
							divisibility: 10,
							symbol: Some('@'),
							spaced_rune: SpacedRune { rune: Rune(0), spacers: 1 },
						},
					)]
					.into()
				},
			...
			.unwrap_err(),
			Error::ZeroValue { output: 0, rune: SpacedRune { rune: Rune(0), spacers: 1 } },
		);
		...
		assert_eq!(
				...
				&Splitfile {
					...
					rune_info: [(
						...
						RuneInfo {
							...
							divisibility: 10,
							symbol: Some('@'),
							spaced_rune: SpacedRune { rune: Rune(0), spacers: 10 },
						},
					)]
					.into()
				},
			...
			.unwrap_err(),
			Error::ZeroValue { output: 1, rune: SpacedRune { rune: Rune(0), spacers: 10 } },
		);
	}
	...
	fn wallet_must_have_enough_runes() {
		assert_eq!(
				...
				&Splitfile {
					...
					rune_info: [(
						...
						RuneInfo {
							...
							divisibility: 10,
							symbol: Some('@'),
							spaced_rune: SpacedRune { rune: Rune(0), spacers: 2 },
						},
					)]
					.into(),
				},
			...
			.unwrap_err(),
			Error::Shortfall {
				rune: SpacedRune { rune: Rune(0), spacers: 2 },
				have: Pile { amount: 0, divisibility: 10, symbol: Some('@') },
				need: Pile { amount: 1000, divisibility: 10, symbol: Some('@') },
			},
		);
		...
		assert_eq!(
			...
			.unwrap_err(),
			Error::Shortfall {
				rune: SpacedRune { rune: Rune(0), spacers: 1 },
				have: Pile { amount: 1000, divisibility: 2, symbol: Some('x') },
				need: Pile { amount: 2000, divisibility: 2, symbol: Some('x') },
			},
		);
	}
				...
				output: vec![
					TxOut {
						...
						script_pubkey: Runestone {
							edicts: vec![Edict { id, amount: 1000, output: 1 }],
							etching: None,
							mint: None,
							pointer: None,
						}
					...
					TxOut { script_pubkey: address.into(), value: Amount::from_sat(294) }
				],
			},
		);
	}
			...
			Transaction {
				...
				output: vec![
					TxOut {
						value: Amount::from_sat(0),
						script_pubkey: Runestone {
							edicts: vec![Edict { id, amount: 1000, output: 2 }],
							etching: None,
							mint: None,
							pointer: None,
						}
					...
					TxOut { script_pubkey: change.into(), value: TARGET_POSTAGE },
					TxOut { script_pubkey: address.into(), value: Amount::from_sat(294) }
				],
			},
		);
		...
		pretty_assert_eq!(
			...
			Transaction {
				...
				output: vec![
					...
					TxOut { script_pubkey: change.into(), value: Amount::from_sat(500) },
					TxOut { script_pubkey: address.into(), value: Amount::from_sat(294) }
				],
			},
		);
			...
			Transaction {
				...
				output: vec![
					TxOut {
						value: Amount::from_sat(0),
						script_pubkey: Runestone {
							edicts: vec![Edict { id: rune_id(0), amount: 1000, output: 2 }],
							etching: None,
							mint: None,
							pointer: None,
						}
		...
		pretty_assert_eq!(
			...
			Transaction {
				...
				output: vec![
					...
					TxOut { script_pubkey: address.into(), value: Amount::from_sat(330) }
				],
			},
		);
	}
	...
	fn excessive_inputs_are_not_selected() {
		...
		let balances =
			[(output, [(rune, 1000)].into()), (outpoint(1), [(rune, 1000)].into())].into();
	...
	fn multiple_inputs_may_be_selected() {
		...
		let balances =
			[(outpoint(0), [(rune, 1000)].into()), (outpoint(1), [(rune, 1000)].into())].into();
			...
			Transaction {
				...
				output: vec![
					TxOut {
						value: Amount::from_sat(0),
						script_pubkey: Runestone {
							edicts: vec![Edict { id, amount: 2000, output: 1 }],
							etching: None,
							mint: None,
							pointer: None,
						}
	...
	fn two_outputs_no_change() {
		...
		let splits = Splitfile {
			outputs: vec![
				splitfile::Output { address: address(0), runes: [(rune, 500)].into(), value: None },
				splitfile::Output { address: address(1), runes: [(rune, 500)].into(), value: None },
			],
			rune_info: [(
				rune,
				RuneInfo {
			...
			Transaction {
				...
				output: vec![
					TxOut {
						...
						script_pubkey: Runestone {
							edicts: vec![
								Edict { id, amount: 500, output: 1 },
								Edict { id, amount: 500, output: 2 }
							],
							etching: None,
							mint: None,
							pointer: None,
					...
					TxOut { script_pubkey: address(0).into(), value: Amount::from_sat(294) },
					TxOut { script_pubkey: address(1).into(), value: Amount::from_sat(294) }
				],
			},
		);
	}
	...
	fn outputs_may_receive_multiple_runes() {
		...
		let balances =
			[(outpoint(0), [(Rune(0), 1000)].into()), (outpoint(1), [(Rune(1), 2000)].into())]
				.into();
			...
			rune_info: [
				...
				(
					...
					RuneInfo {
						...
						divisibility: 0,
						symbol: None,
						spaced_rune: SpacedRune { rune: Rune(1), spacers: 0 },
					},
				),
			]
			.into(),
			...
			Transaction {
				...
				output: vec![
					TxOut {
						...
						script_pubkey: Runestone {
							edicts: vec![
								Edict { id: rune_id(0), amount: 1000, output: 1 },
								Edict { id: rune_id(1), amount: 2000, output: 1 },
							],
							etching: None,
							mint: None,
							pointer: None,
			...
			Transaction {
				...
				output: (0..11)
					.map(|i| if i == 0 {
						...
						TxOut { script_pubkey: address(i - 1).into(), value: Amount::from_sat(294) }
					})
					.collect()
			}
		);
