...
impl Burn {
	pub(crate) fn run(self, wallet: Wallet) -> SubcommandResult {
		let (unsigned_transaction, burn_amount) = match self.asset {
			Outgoing::InscriptionId(id) => {
				...
				ensure!(inscription_info.value.is_some(), "Cannot burn unbound inscription");
				...
				let push: &script::PushBytes =
					metadata.as_slice().try_into().with_context(|| {
						format!("metadata length {} over maximum {}", metadata.len(), u32::MAX)
					})?;
		...
		Ok(Some(Box::new(send::Output { txid, psbt, asset: self.asset, fee })))
	}
	...
	fn create_unsigned_burn_satpoint_transaction(
		...
		ensure!(!runic_outputs.contains(&satpoint.outpoint), "runic outpoints may not be burned");
