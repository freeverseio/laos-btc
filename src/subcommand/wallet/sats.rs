...
pub(crate) struct Sats {
	...
	tsv: Option<PathBuf>,
	#[arg(long, conflicts_with = "tsv", help = "Display list of all sat ranges in wallet.")]
	all: bool,
}
...
impl Sats {
	pub(crate) fn run(&self, wallet: Wallet) -> SubcommandResult {
		ensure!(wallet.has_sat_index(), "sats requires index created with `--index-sats` flag");
		...
		if self.all {
			...
			for (outpoint, sat, offset, rarity) in Self::rare_sats(haystacks) {
				output.push(OutputRare { sat, output: outpoint, offset, rarity });
			}
	...
	fn find(
		...
		while i < needles.len() && j < haystacks.len() {
			...
			if needle >= start && needle < end {
				results.insert(
					value.into(),
					SatPoint { outpoint: *outpoint, offset: offset + needle.0 - start },
				);
			}
...
mod tests {
	...
	fn case(tsv: &str, haystacks: &[(OutPoint, Vec<(u64, u64)>)], expected: &[(&str, SatPoint)]) {
		assert_eq!(
			...
			expected.iter().map(|(sat, satpoint)| (sat.to_string(), *satpoint)).collect()
		);
	}
	...
	fn identify_from_tsv_single() {
		case("0\n", &[(outpoint(1), vec![(0, 1)])], &[("0", satpoint(1, 0))]);
	}
