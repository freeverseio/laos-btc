...
pub(crate) struct Mint {
	...
	rune: SpacedRune,
	#[clap(long, help = "Include <AMOUNT> postage with mint output. [default: 10000sat]")]
...
impl Mint {
	pub(crate) fn run(self, wallet: Wallet) -> SubcommandResult {
		...
		let runestone = Runestone { mint: Some(id), ..default() };
		...
		let unfunded_transaction = Transaction {
			...
			input: Vec::new(),
			output: vec![
				TxOut { script_pubkey, value: Amount::from_sat(0) },
		...
		assert_eq!(Runestone::decipher(&signed_transaction), Some(Artifact::Runestone(runestone)),);

		let transaction = bitcoin_client.send_raw_transaction(&signed_transaction)?;

		Ok(Some(Box::new(Output {
			rune: self.rune,
			pile: Pile { amount, divisibility: rune_entry.divisibility, symbol: rune_entry.symbol },
			mint: transaction,
		})))
	}
}
