...
impl Batch {
	pub(crate) fn run(self, wallet: Wallet) -> SubcommandResult {
		...
		let (inscriptions, reveal_satpoints, postages, destinations) = batchfile.inscriptions(
			&wallet,
			utxos,
			parent_info.iter().map(|info| info.tx_out.value.to_sat()).collect(),
	...
	fn check_etching(wallet: &Wallet, etching: &batch::Etching) -> Result {
		...
		ensure!(wallet.get_rune(rune)?.is_none(), "rune `{rune}` has already been etched",);
		...
		let mintable = etching
			...
			supply ==
				premine.checked_add(mintable).ok_or_else(|| anyhow!(
					"`premine` + `terms.cap` * `terms.amount` over maximum"
				))?,
      ...
      "rune reveal height below rune activation height: {reveal_height} < {first_rune_height}",
		...
		ensure!(rune >= minimum, "rune is less than minimum for next block: {rune} < {minimum}",);
...
",
    ...
    .unwrap();
		...
		metadata
			.insert(Value::String("title".to_string()), Value::String("Lorem Ipsum".to_string()));
