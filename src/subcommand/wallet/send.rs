...
impl Send {
	pub(crate) fn run(self, wallet: Wallet) -> SubcommandResult {
		let address = self.address.clone().require_network(wallet.chain().network())?;

		let unsigned_transaction = match self.asset {
			Outgoing::Amount(amount) =>
				wallet.create_unsigned_send_amount_transaction(address, amount, self.fee_rate)?,
			Outgoing::Rune { decimal, rune } => wallet
				.create_unsigned_send_or_burn_runes_transaction(
					Some(address),
					rune,
					decimal,
					self.postage,
		...
		Ok(Some(Box::new(Output { txid, psbt, asset: self.asset, fee })))
	}
}
