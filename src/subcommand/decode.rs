...
pub(crate) struct Decode {
	...
	file: Option<PathBuf>,
	#[arg(long, help = "Serialize inscriptions in a compact, human-readable format.")]
	compact: bool,
}
...
impl Decode {
	pub(crate) fn run(self, settings: Settings) -> SubcommandResult {
		let transaction = if let Some(txid) = self.txid {
			settings.bitcoin_rpc_client(None)?.get_raw_transaction(&txid, None)?
		...
		if self.compact {
			...
			Ok(Some(Box::new(RawOutput { inscriptions, runestone })))
		}
	}
}
