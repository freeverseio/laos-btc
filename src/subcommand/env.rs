...
",
		...
		let yaml = serde_yaml::to_string(&batch::File {
			etching: Some(batch::Etching {
				...
				symbol: '¢',
				terms: Some(batch::Terms { amount: "1000".parse().unwrap(), cap: 1, ..default() }),
				turbo: false,
			}),
		...
		if !absolute.join("regtest/wallets/ord").try_exists()? {
			...
			ensure!(output.status.success(), "failed to generate receive address: {status}");
		...
		let datadir = if relative.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
			relative
		} else {
			format!("'{relative}'")
		};
