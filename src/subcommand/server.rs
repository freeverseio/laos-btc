...
pub struct Server {
	#[arg(long, help = "Listen on <ADDRESS> for incoming requests. [default: 0.0.0.0]")]
	...
	#[arg(long, help = "Listen on <HTTP_PORT> for incoming HTTP requests. [default: 80]")]
	...
	#[arg(long, default_value = "5s", help = "Poll Bitcoin Core every <POLLING_INTERVAL>.")]
...
impl Server {
	pub fn run(self, settings: Settings, index: Arc<Index>, handle: Handle) -> SubcommandResult {
		Runtime::new()?.block_on(async {
			...
			let router = Router::new()
				...
				.route("/inscription/:inscription_query/:child", get(Self::inscription_child))
				...
				.route("/r/blockhash/:height", get(Self::block_hash_from_height_json))
				.route("/r/blockheight", get(Self::block_height))
				.route("/r/blocktime", get(Self::block_time))
				.route("/r/blockinfo/:query", get(Self::block_info))
				.route("/r/inscription/:inscription_id", get(Self::inscription_recursive))
				...
				.route("/r/undelegated-content/:inscription_id", get(Self::undelegated_content))
				.route("/r/metadata/:inscription_id", get(Self::metadata))
				.route("/r/parents/:inscription_id", get(Self::parents_recursive))
				.route("/r/parents/:inscription_id/:page", get(Self::parents_recursive_paginated))
				.route("/r/sat/:sat_number", get(Self::sat_inscriptions))
				.route("/r/sat/:sat_number/:page", get(Self::sat_inscriptions_paginated))
				.route("/r/sat/:sat_number/at/:index", get(Self::sat_inscription_at_index))
				...
				.layer(CorsLayer::new().allow_methods([http::Method::GET]).allow_origin(Any))
			...
			match (self.http_port(), self.https_port()) {
				(Some(http_port), None) =>
					self.spawn(&settings, router, handle, http_port, SpawnConfig::Http)?.await??,
	...
	fn acme_domains(&self) -> Result<Vec<String>> {
		if !self.acme_domain.is_empty() {
			...
			Ok(vec![System::host_name().ok_or(anyhow!("no hostname found"))?])
		}
	}
	...
	fn acceptor(&self, settings: &Settings) -> Result<AxumAcceptor> {
		let config = AcmeConfig::new(self.acme_domains()?)
			...
			.cache_option(Some(DirCache::new(Self::acme_cache(self.acme_cache.as_ref(), settings))))
		...
		task::block_in_place(|| {
			...
			let satpoint = index.rare_sat_satpoint(sat)?.or_else(|| {
				inscriptions.first().and_then(|&first_inscription_id| {
					index.get_inscription_satpoint_by_id(first_inscription_id).ok().flatten()
				})
			});
			...
			let address = if let Some(satpoint) = satpoint {
				if satpoint.outpoint == unbound_outpoint() {
					...
					server_config.chain.address_from_script(&tx_out.script_pubkey).ok()
				}
			} else {
				None
			};
			...
			Ok(if accept_json {
				...
				SatHtml { address, blocktime, inscriptions, sat, satpoint }
					.page(server_config)
					.into_response()
			})
		})
	...
	async fn satpoint(
		...
		task::block_in_place(|| {
			...
			Err(ServerError::NotFound(format!("satpoint {satpoint} not found")))
		})
	}
	...
	async fn outputs_address(
		...
		task::block_in_place(|| {
			if !index.has_address_index() {
				return Err(ServerError::NotFound("this server has no address index".to_string()));
			}
			...
			for output in outputs.into_iter() {
				let include = match output_type {
					...
					OutputType::Inscribed => !index
						...
						.is_empty(),
					OutputType::Runic =>
						!index.get_rune_balances_for_output(output)?.unwrap_or_default().is_empty(),
				};
	...
	async fn rune(
		...
		task::block_in_place(|| {
			if !index.has_rune_index() {
				return Err(ServerError::NotFound("this server has no rune index".to_string()));
			}

			let rune = match rune_query {
				query::Rune::Spaced(spaced_rune) => spaced_rune.rune,
				query::Rune::Id(rune_id) =>
					index.get_rune_by_id(rune_id)?.ok_or_not_found(|| format!("rune {rune_id}"))?,
			...
			Ok(if accept_json {
				...
				RuneHtml { entry, id, mintable, parent }.page(server_config).into_response()
			})
		})
	}
	...
	async fn runes(
		...
		Self::runes_paginated(Extension(server_config), Extension(index), Path(0), accept_json)
			.await
	}
	...
	async fn runes_paginated(
		...
		task::block_in_place(|| {
			...
			Ok(if accept_json {
				Json(RunesHtml { entries, more, prev, next }).into_response()
			} else {
				RunesHtml { entries, more, prev, next }.page(server_config).into_response()
			})
		})
	}
	...
	async fn blocks(
		...
		task::block_in_place(|| {
			...
			Ok(if accept_json {
				...
				BlocksHtml::new(blocks, featured_blocks).page(server_config).into_response()
			})
		})
	}
	...
	async fn address(
		...
		task::block_in_place(|| {
			...
			Ok(if accept_json {
				Json(api::AddressInfo { sat_balance, outputs, inscriptions, runes_balances })
					.into_response()
			} else {
				AddressHtml { address, outputs, inscriptions, sat_balance, runes_balances }
					.page(server_config)
					.into_response()
			})
		})
	...
	async fn transaction(
		...
		task::block_in_place(|| {
			let transaction =
				index.get_transaction(txid)?.ok_or_not_found(|| format!("transaction {txid}"))?;
	...
	async fn decode(
		...
		task::block_in_place(|| {
			...
			Ok(if accept_json {
				Json(api::Decode { inscriptions, runestone }).into_response()
			} else {
				StatusCode::NOT_FOUND.into_response()
			})
		})
		...
		task::block_in_place(|| {
			let Some(inscription) = index.get_inscription_by_id(inscription_id)? else {
				return if let Some(proxy) = server_config.proxy.as_ref() {
					...
					Err(ServerError::NotFound(format!("inscription {} not found", inscription_id)))
				};
			};
	...
	async fn inscription_recursive(
		...
		task::block_in_place(|| {
			...
			let satpoint =
				index.get_inscription_satpoint_by_id(inscription_id).ok().flatten().unwrap();
	...
	async fn favicon() -> ServerResult {
		Ok(Self::static_asset(Path("/favicon.png".to_string())).await.into_response())
	}
		...
		task::block_in_place(|| {
			...
			Ok((
				[
					...
					(header::CONTENT_SECURITY_POLICY, "default-src 'unsafe-inline'"),
				],
	...
	async fn block_height(Extension(index): Extension<Arc<Index>>) -> ServerResult<String> {
		task::block_in_place(|| {
			Ok(index.block_height()?.ok_or_not_found(|| "blockheight")?.to_string())
		})
	}

	async fn block_hash(Extension(index): Extension<Arc<Index>>) -> ServerResult<String> {
		task::block_in_place(|| {
			Ok(index.block_hash(None)?.ok_or_not_found(|| "blockhash")?.to_string())
		})
	}
	...
	async fn block_hash_json(
		...
		task::block_in_place(|| {
			Ok(Json(index.block_hash(None)?.ok_or_not_found(|| "blockhash")?.to_string()))
		})
	}
	...
	async fn block_hash_from_height(
		...
		task::block_in_place(|| {
			Ok(index.block_hash(Some(height))?.ok_or_not_found(|| "blockhash")?.to_string())
		})
	}
	...
	async fn block_hash_from_height_json(
		...
		task::block_in_place(|| {
			Ok(Json(index.block_hash(Some(height))?.ok_or_not_found(|| "blockhash")?.to_string()))
		})
	}
	...
	async fn block_info(
		...
		task::block_in_place(|| {
			let hash = match query {
				...
				query::Block::Height(height) =>
					index.block_hash(Some(height))?.ok_or_not_found(|| format!("block {height}"))?,
			};

			let header = index.block_header(hash)?.ok_or_not_found(|| format!("block {hash}"))?;

			let info =
				index.block_header_info(hash)?.ok_or_not_found(|| format!("block {hash}"))?;
			...
			Ok(Json(api::BlockInfo {
				...
				median_time: info.median_time.map(|median_time| median_time.try_into().unwrap()),
	...
	async fn input(
		...
		task::block_in_place(|| {
			...
			let block = index.get_block_by_height(path.0)?.ok_or_not_found(not_found)?;

			let transaction = block.txdata.into_iter().nth(path.1).ok_or_not_found(not_found)?;

			let input = transaction.input.into_iter().nth(path.2).ok_or_not_found(not_found)?;
	...
	fn proxy(proxy: &Url, path: &String) -> ServerResult<Response> {
		...
		Ok((response.status(), headers, response.bytes().map_err(|err| anyhow!(err))?)
			.into_response())
	}
	...
	async fn content(
		...
		task::block_in_place(|| {
			...
			Ok(Self::content_response(inscription, accept_encoding, &server_config)?
	...
	fn content_response(
		...
		if let Some(content_encoding) = inscription.content_encoding() {
			if accept_encoding.is_acceptable(&content_encoding) {
				...
				return Err(ServerError::NotAcceptable { accept_encoding, content_encoding });
			}
		}
	...
	async fn preview(
		...
		task::block_in_place(|| {
			...
			if let Media::Iframe = media {
				return Ok(Self::content_response(inscription, accept_encoding, &server_config)?
					.ok_or_not_found(|| format!("inscription {inscription_id} content"))?
					.into_response());
			}
			...
			match media {
				Media::Audio =>
					Ok((content_security_policy, PreviewAudioHtml { inscription_id })
						.into_response()),
				Media::Code(language) =>
					Ok((content_security_policy, PreviewCodeHtml { inscription_id, language })
						.into_response()),
				Media::Font =>
					Ok((content_security_policy, PreviewFontHtml { inscription_id })
						.into_response()),
				Media::Iframe => unreachable!(),
				Media::Image(image_rendering) => Ok((
					content_security_policy,
					PreviewImageHtml { image_rendering, inscription_id },
				)
					.into_response()),
				Media::Markdown =>
					Ok((content_security_policy, PreviewMarkdownHtml { inscription_id })
						.into_response()),
				Media::Model =>
					Ok((content_security_policy, PreviewModelHtml { inscription_id })
						.into_response()),
				Media::Pdf =>
					Ok((content_security_policy, PreviewPdfHtml { inscription_id }).into_response()),
				Media::Text =>
					Ok((content_security_policy, PreviewTextHtml { inscription_id })
						.into_response()),
				Media::Unknown => Ok((content_security_policy, PreviewUnknownHtml).into_response()),
				Media::Video =>
					Ok((content_security_policy, PreviewVideoHtml { inscription_id })
						.into_response()),
			}
		})
	}
	...
	async fn inscription_child(
		...
		Extension(index): Extension<Arc<Index>>,
		AcceptJson(accept_json): AcceptJson,
		Path((DeserializeFromStr(query), child)): Path<(
			DeserializeFromStr<query::Inscription>,
			usize,
		)>,
	) -> ServerResult {
	...
	async fn inscription_inner(
		...
		task::block_in_place(|| {
			...
			Ok(if accept_json {
				...
				InscriptionHtml {
					chain: server_config.chain,
					charms: Charm::Vindicated.unset(info.charms.iter().fold(
						0,
						|mut acc, charm| {
							charm.set(&mut acc);
							acc
	...
	async fn collections_paginated(
		...
		task::block_in_place(|| {
			...
			Ok(CollectionsHtml { inscriptions: collections, prev, next }
				.page(server_config)
				.into_response())
		})
	}
	...
	async fn children_paginated(
		...
		task::block_in_place(|| {
			...
			Ok(ChildrenHtml { parent, parent_number, children, prev_page, next_page }
				.page(server_config)
				.into_response())
		})
	}
		...
		task::block_in_place(|| {
			let Some(parent) = index.get_inscription_entry(parent)? else {
				return if let Some(proxy) = server_config.proxy.as_ref() {
					...
					Err(ServerError::NotFound(format!("inscription {} not found", parent)))
				};
			};
	...
	async fn child_inscriptions_recursive(
		...
		Self::child_inscriptions_recursive_paginated(Extension(index), Path((inscription_id, 0)))
			.await
	}
	...
	async fn child_inscriptions_recursive_paginated(
		...
		task::block_in_place(|| {
			...
			Ok(Json(api::ChildInscriptions { children, more, page }).into_response())
		})
	}
	...
	async fn inscriptions_paginated(
		...
		task::block_in_place(|| {
			...
			Ok(if accept_json {
				Json(api::Inscriptions { ids: inscriptions, page_index, more }).into_response()
			} else {
	...
	async fn parents_paginated(
		...
		task::block_in_place(|| {
			...
			let (parents, more) =
				index.get_parents_by_sequence_number_paginated(child.parents, page)?;

			let prev_page = page.checked_sub(1);

			let next_page = more.then_some(page + 1);

			Ok(ParentsHtml { id, number: child.inscription_number, parents, prev_page, next_page }
				.page(server_config)
				.into_response())
		})
	}
	...
	async fn parents_recursive_paginated(
		...
		task::block_in_place(|| {
			...
			Ok(Json(api::Inscriptions { ids, more, page_index }).into_response())
		})
	}
	...
	async fn sat_inscriptions_paginated(
		...
		task::block_in_place(|| {
			if !index.has_sat_index() {
				return Err(ServerError::NotFound("this server has no sat index".to_string()));
			}
...
mod tests {
	...
	impl TestServer {
		...
		pub(crate) fn etch(
			...
			(txid, RuneId { block: (self.index.block_count().unwrap() - 1).into(), tx: 1 })
		}
		...
		fn assert_response_regex(
			...
			let response = self.get(path);
			assert_eq!(response.status(), status, "response: {}", response.text().unwrap());
		...
		match Arguments::try_parse_from(args.split_whitespace()) {
			Ok(arguments) => match arguments.subcommand {
				Subcommand::Server(server) =>
					(Settings::from_options(arguments.options).or_defaults().unwrap(), server),
	...
	fn https_sets_https_port_to_443() {
		assert_eq!(
			parse_server_args(
				"ord server --https --acme-cache foo --acme-contact bar --acme-domain baz"
			)
			.1
			.https_port(),
			Some(443)
	...
	fn acme_cache_defaults_to_data_dir() {
		...
		let settings = Settings::from_options(arguments.options).or_defaults().unwrap();

		let acme_cache = Server::acme_cache(None, &settings).display().to_string();
		assert!(
			acme_cache.contains(if cfg!(windows) { r"foo\acme-cache" } else { "foo/acme-cache" }),
			"{acme_cache}"
		)
	}
	...
	fn acme_cache_flag_is_respected() {
		...
		let acme_cache = Server::acme_cache(Some(&"bar".into()), &settings).display().to_string();
	...
	fn acme_domain_defaults_to_hostname() {
		...
		assert_eq!(server.acme_domains().unwrap(), &[System::host_name().unwrap()]);
	}
	...
	fn search_by_rune_id_returns_rune() {
		...
		server.etch(
			Runestone {
				edicts: vec![Edict { id: RuneId::default(), amount: u128::MAX, output: 0 }],
				etching: Some(Etching { rune: Some(rune), ..default() }),
				..default()
			},
			1,
			None,
	...
	fn search_by_satpoint_returns_sat() {
		...
		server.assert_redirect(&format!("/search/{txid}:0:0"), &format!("/satpoint/{txid}:0:0"));
	...
	fn rune_not_etched_shows_unlock_height() {
		...
		server.assert_html_status(
			"/rune/A",
			StatusCode::NOT_FOUND,
			RuneNotFoundHtml { rune: Rune(0), unlock_height: Some(Height(209999)) },
		);
	}
	...
	fn runes_are_displayed_on_runes_page() {
		...
		server.assert_html(
			"/runes",
			RunesHtml { entries: Vec::new(), more: false, prev: None, next: None },
		);
	...
	fn runes_are_displayed_on_rune_page() {
		...
		server.assert_html(
			format!("/rune/{rune}"),
			RuneHtml { id, entry, mintable: false, parent: Some(parent) },
		);
...
.*",
	...
	fn etched_runes_are_displayed_on_block_page() {
		...
		let (_txid, id) = server.etch(
			Runestone {
				edicts: vec![Edict { id: RuneId::default(), amount: u128::MAX, output: 0 }],
				etching: Some(Etching { rune: Some(rune0), ..default() }),
				..default()
			},
			1,
			None,
		...
		assert_eq!(server.index.get_runes_in_block(id.block - 1).unwrap().len(), 0);
...
.*",
	...
	fn runes_are_displayed_on_output_page() {
		...
		assert_eq!(server.index.get_rune_balances().unwrap(), [(output, vec![(id, u128::MAX)])]);
...
.*"
		...
		pretty_assert_eq!(
			...
			api::Output {
				...
				runes: Some(
					vec![(
						SpacedRune { rune: Rune(RUNE), spacers: 0 },
						Pile {
	...
	fn http_to_https_redirect_with_path() {
		TestServer::builder()
			...
			.https()
			.build()
			.assert_redirect("/sat/0", &format!("https://{}/sat/0", System::host_name().unwrap()));
	}
...
<div class=block>
  ...
  background-color: var\(--rare\);
	...
	fn block_by_hash() {
		...
		test_server.mine_blocks(1);
		let transaction =
			TransactionTemplate { inputs: &[(1, 0, 0, Default::default())], fee: 0, ..default() };
...
",
	...
	fn outputs_traversed_are_tracked() {
		...
		assert_eq!(server.index.statistic(crate::index::Statistic::OutputsTraversed), 1);
		...
		assert_eq!(server.index.statistic(crate::index::Statistic::OutputsTraversed), 3);
	}
	...
	fn coinbase_sat_ranges_are_tracked() {
		...
		assert_eq!(server.index.statistic(crate::index::Statistic::SatRanges), 1);

		server.mine_blocks(1);

		assert_eq!(server.index.statistic(crate::index::Statistic::SatRanges), 2);

		server.mine_blocks(1);

		assert_eq!(server.index.statistic(crate::index::Statistic::SatRanges), 3);
	}
	...
	fn split_sat_ranges_are_tracked() {
		...
		assert_eq!(server.index.statistic(crate::index::Statistic::SatRanges), 4,);
	}
	...
	fn fee_sat_ranges_are_tracked() {
		...
		assert_eq!(server.index.statistic(crate::index::Statistic::SatRanges), 5,);
	}
	...
	fn content_security_policy_with_origin() {
		let (headers, _) = Server::content_response(
			...
			AcceptEncoding::default(),
			&ServerConfig { csp_origin: Some("https://ordinals.com".into()), ..default() },
		)
		.unwrap()
		.unwrap();
	...
	fn code_preview() {
		...
		server.assert_response_regex(
			...
			StatusCode::OK,
			format!(
				r".*<html lang=en data-inscription={inscription_id} data-language=javascript>.*"
			),
		);
	}
	...
	fn content_response_no_content_type() {
		let (headers, body) = Server::content_response(
			Inscription { content_type: None, body: Some(Vec::new()), ..default() },
			AcceptEncoding::default(),
			&ServerConfig::default(),
		)
		.unwrap()
	...
	fn iframe_preview() {
		...
		let txid = server.core.broadcast_tx(TransactionTemplate {
			inputs: &[(1, 0, 0, inscription("text/html;charset=utf-8", "hello").to_witness())],
			..default()
		});
	...
	fn error_content_responses_have_max_age_zero_cache_control_headers() {
		let server = TestServer::builder().chain(Chain::Regtest).build();
		let response = server
			.get("/content/6ac5cacb768794f4fd7a78bf00f2074891fce68bd65c4ff36e77177237aacacai0");
...
.*<a href=/inscription/.*><iframe .* src=/preview/.*></iframe></a>.*
	...
	fn inscription_child() {
		...
		let child0 = InscriptionId { txid: child_txid, index: 0 };
...
.*<dd class=collapse>{child0}</dd>.*"
		...
		let child1 = InscriptionId { txid: child_txid, index: 1 };
...
.*<dd class=collapse>{child1}</dd>.*"
	...
	fn inscription_with_parent_page() {
		...
		let parent_a_inscription_id = InscriptionId { txid: parent_a_txid, index: 0 };

		let parent_b_inscription_id = InscriptionId { txid: parent_b_txid, index: 0 };
	...
	fn inscription_parent_page_pagination() {
		...
		let txid = server.core.broadcast_tx(TransactionTemplate { inputs: &inputs, ..default() });
...
"
	...
	fn charm_reinscription_in_same_tx_with_pointer() {
		...
		let reinscription: Inscription =
			InscriptionTemplate { pointer: Some(0), ..default() }.into();
...
"
	...
	fn utxo_recursive_endpoint_all() {
		let server = TestServer::builder().chain(Chain::Regtest).index_sats().index_runes().build();
			...
			api::UtxoRecursive {
				...
				runes: Some(
					[(
						...
						Pile { amount: u128::MAX, divisibility: 1, symbol: None }
					)]
					.into_iter()
					.collect()
				),
	...
	fn sat_recursive_endpoints() {
		...
		assert_eq!(
			...
			api::SatInscriptions { ids: Vec::new(), page: 0, more: false }
		);
		...
		assert_eq!(paginated_response.ids.len(), equivalent_paginated_response.ids.len());
		...
		assert_eq!(
			server.get_json::<api::SatInscription>("/r/sat/5000000000/at/0").id,
			Some(ids[0])
		);

		assert_eq!(
			server.get_json::<api::SatInscription>("/r/sat/5000000000/at/-111").id,
			Some(ids[0])
		);

		assert_eq!(
			server.get_json::<api::SatInscription>("/r/sat/5000000000/at/110").id,
			Some(ids[110])
		);

		assert_eq!(
			server.get_json::<api::SatInscription>("/r/sat/5000000000/at/-1").id,
			Some(ids[110])
		);

		assert!(server.get_json::<api::SatInscription>("/r/sat/5000000000/at/111").id.is_none());
	}
	...
	fn parents_recursive_endpoint() {
		...
		let parents_json =
			server.get_json::<api::Inscriptions>(format!("/r/parents/{inscription_id}"));
	...
	fn child_inscriptions_recursive_endpoint() {
		...
		assert_eq!(child_inscriptions_json.children[0].id, first_child_inscription_id);
		assert_eq!(child_inscriptions_json.children[0].number, 1); // parent is #0, 1st child is #1

		assert_eq!(child_inscriptions_json.children[99].id, hundredth_child_inscription_id);
		...
		assert_eq!(child_inscriptions_json.children[0].id, hundred_first_child_inscription_id);
		assert_eq!(child_inscriptions_json.children[0].number, -100);

		assert_eq!(child_inscriptions_json.children[10].id, hundred_eleventh_child_inscription_id);
	...
	fn inscription_not_found() {
		TestServer::builder().chain(Chain::Regtest).build().assert_response(
			"/inscription/0",
			StatusCode::NOT_FOUND,
	...
	fn delegate() {
		...
		let inscription = Inscription { delegate: Some(delegate.value()), ..default() };
	...
	fn undelegated_content() {
		...
		let delegate_id = InscriptionId { txid: delegate_txid, index: 0 };
		...
		server.assert_response(format!("/r/undelegated-content/{id}"), StatusCode::OK, "bar");
		...
		let normal_id = InscriptionId { txid: normal_txid, index: 0 };
	...
	fn children_proxy() {
		...
		let parent_id = InscriptionId { txid: parent_txid, index: 0 };
		...
		let children =
			server_with_proxy.get_json::<api::Children>(format!("/r/children/{parent_id}"));
	...
	fn inscription_proxy() {
		...
		pretty_assert_eq!(
			...
			api::InscriptionRecursive {
				...
				sat: None,
				satpoint: SatPoint { outpoint: OutPoint { txid, vout: 0 }, offset: 0 },
				timestamp: 2,
	...
	fn inscriptions_can_be_hidden_with_config() {
		let core = mockcore::builder().network(Chain::Regtest.network()).build();
	...
	fn burned_charm() {
		...
		let txid = server.core.broadcast_tx(TransactionTemplate {
			...
			op_return: Some(
				script::Builder::new().push_opcode(opcodes::all::OP_RETURN).into_script(),
			),
			..default()
		});
	...
	fn burned_charm_on_transfer() {
		...
		let id = InscriptionId { txid: create_txid, index: 0 };
		...
		pretty_assert_eq!(
			...
			api::InscriptionRecursive {
				...
				id,
				number: 0,
				output: OutPoint { txid: create_txid, vout: 0 },
				sat: None,
				satpoint: SatPoint { outpoint: OutPoint { txid: create_txid, vout: 0 }, offset: 0 },
				timestamp: 2,
				value: Some(50 * COIN_VALUE),
				address: Some("bcrt1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqdku202".to_string())
			}
		...
		pretty_assert_eq!(
			...
			api::InscriptionRecursive {
				...
				id,
				number: 0,
				output: OutPoint { txid: transfer_txid, vout: 0 },
				sat: None,
				satpoint: SatPoint {
