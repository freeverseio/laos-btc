...
impl Updater<'_> {
	pub(crate) fn update_index(&mut self, mut wtx: WriteTransaction) -> Result {
		...
		wtx.open_table(WRITE_TRANSACTION_STARTING_BLOCK_COUNT_TO_TIMESTAMP)?.insert(
			&self.height,
			&SystemTime::now()
	...
	fn get_block_with_retries(
		...
		let mut errors = 0;
		loop {
			match client.get_block_hash(height.into()).into_option().and_then(|option| {
				option
					.map(|hash| {
	...
	fn spawn_fetcher(
		index: &Index,
	) -> Result<(mpsc::Sender<OutPoint>, broadcast::Receiver<TxOut>)> {
		...
		let (outpoint_sender, mut outpoint_receiver) =
			mpsc::channel::<OutPoint>(CHANNEL_BUFFER_SIZE);

		let (txout_sender, txout_receiver) = broadcast::channel::<TxOut>(CHANNEL_BUFFER_SIZE);

		// Default rpcworkqueue in bitcoind is 16, meaning more than 16 concurrent requests will be
		// rejected. Since we are already requesting blocks on a separate thread, and we don't
		// want to break if anything else runs a request, we keep this to 12.
		...
		thread::spawn(move || {
			...
			rt.block_on(async move {
				loop {
					...
					// std::sync::mpsc::Receiver. So we just loop until BATCH_SIZE doing
					// try_recv until it returns None.
	...
	fn index_utxo_entries<'wtx>(
		...
		let mut sequence_number_to_children =
			wtx.open_multimap_table(SEQUENCE_NUMBER_TO_CHILDREN)?;
		...
		if !self.index.have_full_utxo_index() {
			...
			let txids = block.txdata.iter().map(|(_, txid)| txid).collect::<HashSet<_>>();
			...
			.chain(block.txdata.iter().enumerate().take(1))
		{
				...
				tx.input
					...
					.map(|input| {
						...
						let entry = if let Some(entry) =
							utxo_cache.remove(&OutPoint::load(outpoint))
						{
							...
							if self.index.index_addresses {
								entry
									.push_script_pubkey(txout.script_pubkey.as_bytes(), self.index);
							}
			...
			let mut output_utxo_entries =
				tx.output.iter().map(|_| UtxoEntryBuf::new()).collect::<Vec<UtxoEntryBuf>>();
			...
			if self.index.index_sats {
				...
				if tx_offset == 0 {
					...
					input_sat_ranges =
						Some(input_utxo_entries.iter().map(|entry| entry.sat_ranges()).collect());
		...
		if !lost_sat_ranges.is_empty() {
			...
			let utxo_entry =
				utxo_cache.entry(OutPoint::null()).or_insert(UtxoEntryBuf::empty(self.index));
			...
			for chunk in lost_sat_ranges.chunks_exact(11) {
				...
				if !Sat(start).common() {
					sat_to_satpoint.insert(
						&start,
						&SatPoint { outpoint: OutPoint::null(), offset: lost_sats }.store(),
					)?;
				}
		...
		statistic_to_count.insert(
			...
			&if self.index.index_sats { lost_sats } else { inscription_updater.lost_sats },
		)?;
	...
	fn index_transaction_sats(
		...
		let mut input_sat_ranges_iter =
			input_sat_ranges.iter().flat_map(|slice| slice.chunks_exact(11));
		...
		let mut sats =
			Vec::with_capacity(input_sat_ranges.iter().map(|slice| slice.len()).sum::<usize>());

		for (vout, output) in tx.output.iter().enumerate() {
			let outpoint = OutPoint { vout: vout.try_into().unwrap(), txid };
			...
			while remaining > 0 {
				...
				if !Sat(range.0).common() {
					sat_to_satpoint.insert(
						&range.0,
						&SatPoint { outpoint, offset: output.value.to_sat() - remaining }.store(),
					)?;
				}
	...
	fn commit(
...

		{
			...
			let mut script_pubkey_to_outpoint =
				wtx.open_multimap_table(SCRIPT_PUBKEY_TO_OUTPOINT)?;
			...
			for (outpoint, mut utxo_entry) in utxo_cache {
				if Index::is_special_outpoint(outpoint) {
					if let Some(old_entry) = outpoint_to_utxo_entry.get(&outpoint.store())? {
						utxo_entry =
							UtxoEntryBuf::merged(old_entry.value(), &utxo_entry, self.index);
					}
				}
