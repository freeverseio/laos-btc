...
impl Context {
	...
	pub(crate) fn configurations() -> Vec<Context> {
		vec![Context::builder().build(), Context::builder().arg("--index-sats").build()]
	}
	...
	pub(crate) fn etch(&self, runestone: Runestone, outputs: usize) -> (Txid, RuneId) {
		...
		if let Some(etching) = runestone.etching {
			let tapscript = script::Builder::new()
				.push_slice::<&PushBytes>(
					etching.rune.unwrap().commitment().as_slice().try_into().unwrap(),
				)
				.into_script();
		...
		(
			...
			RuneId {
				block: u64::try_from(
					block_count + usize::from(Runestone::COMMIT_CONFIRMATIONS) + 1,
				)
				.unwrap(),
				tx: 1,
			},
