...
impl UtxoEntry {
	pub fn parse(&self, index: &Index) -> ParsedUtxoEntry {
		...
		ParsedUtxoEntry { sats, script_pubkey, inscriptions }
	}
...
impl<'a> ParsedUtxoEntry<'a> {
	...
	pub fn parse_inscriptions(&self) -> Vec<(u32, u64)> {
		...
		while byte_offset < inscriptions.len() {
			let sequence_number =
				u32::from_le_bytes(inscriptions[byte_offset..byte_offset + 4].try_into().unwrap());
			byte_offset += 4;

			let (satpoint_offset, varint_len) =
				varint::decode(&inscriptions[byte_offset..]).unwrap();
