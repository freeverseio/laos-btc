...
impl InscriptionUpdater<'_, '_> {
	pub(super) fn index_inscriptions(
		...
		for (input_index, txin) in tx.input.iter().enumerate() {
			...
			for (sequence_number, old_satpoint_offset) in transferred_inscriptions {
				let old_satpoint =
					SatPoint { outpoint: txin.previous_output, offset: old_satpoint_offset };

				let inscription_id = InscriptionEntry::load(
					self.sequence_number_to_entry.get(sequence_number)?.unwrap().value(),
				)
				.id;
				...
				floating_inscriptions.push(Flotsam {
					offset,
					inscription_id,
					origin: Origin::Old { sequence_number, old_satpoint },
				});

				inscribed_offsets.entry(offset).or_insert((inscription_id, 0)).1 += 1;
			}
			...
			while let Some(inscription) = envelopes.peek() {
				...
				let inscription_id = InscriptionId { txid, index: id_counter };
		...
		if index.index_transactions && has_new_inscriptions {
			...
			self.transaction_id_to_transaction
		...
		for flotsam in &mut floating_inscriptions {
			if let Flotsam { origin: Origin::New { parents: purported_parents, .. }, .. } = flotsam
			{
		...
		for flotsam in &mut floating_inscriptions {
			if let Flotsam { origin: Origin::New { ref mut fee, .. }, .. } = flotsam {
		...
		for (vout, txout) in tx.output.iter().enumerate() {
			...
			while let Some(flotsam) = inscriptions.peek() {
				...
				let new_satpoint = SatPoint {
					outpoint: OutPoint { txid, vout: vout.try_into().unwrap() },
	...
	fn calculate_sat(input_sat_ranges: Option<&Vec<&[u8]>>, input_offset: u64) -> Option<Sat> {
		...
		let mut offset = 0;
		for chunk in input_sat_ranges.iter().flat_map(|slice| slice.chunks_exact(11)) {
	...
	fn update_inscription_location(
		...
		let (unbound, sequence_number) = match flotsam.origin {
			Origin::Old { sequence_number, old_satpoint } => {
				if op_return {
					let entry = InscriptionEntry::load(
						self.sequence_number_to_entry.get(&sequence_number)?.unwrap().value(),
					);
					...
					self.sequence_number_to_entry
						.insert(sequence_number, &InscriptionEntry { charms, ..entry }.store())?;
				}
			...
			Origin::New { cursed, fee, hidden, parents, reinscription, unbound, vindicated } => {
				...
				let parent_sequence_numbers = parents
					...
					.map(|parent| {
						let parent_sequence_number =
							self.id_to_sequence_number.get(&parent.store())?.unwrap().value();

						self.sequence_number_to_children
				...
				self.id_to_sequence_number.insert(&inscription_id.store(), sequence_number)?;

				if !hidden {
					self.home_inscriptions.insert(&sequence_number, inscription_id.store())?;
		...
		let satpoint = if unbound {
			let new_unbound_satpoint =
				SatPoint { outpoint: unbound_outpoint(), offset: self.unbound_inscriptions };
		...
		let output_utxo_entry = normal_output_utxo_entry.unwrap_or_else(|| {
			...
			utxo_cache.entry(satpoint.outpoint).or_insert(UtxoEntryBuf::empty(index))
		});
