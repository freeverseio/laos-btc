...
impl RuneUpdater<'_, '_, '_> {
	...
	fn create_rune_entry(
		...
		self.transaction_id_to_rune.insert(&txid.store(), rune.store())?;

		let number = self.runes;
		self.runes += 1;

		self.statistic_to_count.insert(&Statistic::Runes.into(), self.runes)?;
		...
		if let Some(sequence_number) =
			self.inscription_id_to_sequence_number.get(&inscription_id.store())?
		{
			self.sequence_number_to_rune_id.insert(sequence_number.value(), id.store())?;
		}
		...
		for input in &tx.input {
			...
			for instruction in tapscript.instructions() {
				...
				let Some(tx_info) = self
					...
					panic!("can't get input transaction: {}", input.previous_output.txid);
				};
				...
				let confirmations =
					self.height.checked_sub(commit_tx_height.try_into().unwrap()).unwrap() + 1;
	...
	fn unallocated(&mut self, tx: &Transaction) -> Result<HashMap<RuneId, Lot>> {
		...
		for input in &tx.input {
			if let Some(guard) = self.outpoint_to_balances.remove(&input.previous_output.store())? {
