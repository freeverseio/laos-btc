...
impl FromStr for Decimal {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		if let Some((integer, decimal)) = s.split_once('.') {
			...
			let integer = if integer.is_empty() { 0 } else { integer.parse::<u128>()? };
			...
			Ok(Self { value: integer * 10u128.pow(u32::from(scale)) + decimal, scale })
		} else {
			Ok(Self { value: s.parse::<u128>()?, scale: 0 })
		}
	}
}
...
mod tests {
	...
	fn from_str() {
		...
		assert_eq!(".".parse::<Decimal>().unwrap_err().to_string(), "empty decimal",);
	...
	fn to_amount() {
		...
		fn case(s: &str, divisibility: u8, amount: u128) {
			assert_eq!(s.parse::<Decimal>().unwrap().to_integer(divisibility).unwrap(), amount,);
		}

		assert_eq!(
			Decimal { value: 0, scale: 0 }.to_integer(255).unwrap_err().to_string(),
			"divisibility out of range"
		);

		assert_eq!(
			Decimal { value: u128::MAX, scale: 0 }.to_integer(1).unwrap_err().to_string(),
			"amount out of range",
		);

		assert_eq!(
			Decimal { value: 1, scale: 1 }.to_integer(0).unwrap_err().to_string(),
			"excessive precision",
		);
	...
	fn to_string() {
		...
		case(Decimal { value: 101, scale: 2 }, "1.01");
		case(Decimal { value: 1234, scale: 6 }, "0.001234");
		case(Decimal { value: 12, scale: 0 }, "12");
		case(Decimal { value: 12, scale: 1 }, "1.2");
		case(Decimal { value: 12, scale: 2 }, "0.12");
		case(Decimal { value: 123456, scale: 3 }, "123.456");
		case(Decimal { value: 123456789, scale: 6 }, "123.456789");
	}
}
