...
impl TransactionBuilder {
	...
	pub fn build_transaction(self) -> Result<Transaction> {
		if self.change_addresses.len() < 2 {
			return Err(Error::DuplicateAddress(self.change_addresses.first().unwrap().clone()));
		}

		if !self.recipient.is_op_return() {
			let recipient_as_address =
				Address::from_script(self.recipient.as_script(), self.network)?;
			...
			if let Target::Value(output_value) | Target::ExactPostage(output_value) = self.target {
				...
				if output_value < dust_value {
					return Err(Error::Dust { output_value, dust_value });
				}
			}
		}
	...
	fn select_outgoing(mut self) -> Result<Self> {
		...
		self.outputs
			.push(TxOut { script_pubkey: self.recipient.clone(), value: Amount::from_sat(amount) });

		tprintln!("selected outgoing outpoint {} with value {}", self.outgoing.outpoint, amount);
	...
	fn pad_alignment_output(mut self) -> Result<Self> {
		if self.outputs[0].script_pubkey == self.recipient {
			...
			let dust_limit =
				self.unused_change_addresses.last().unwrap().script_pubkey().minimal_non_dust();

			if self.outputs[0].value >= dust_limit {
				tprintln!("no padding needed");
			} else {
				while self.outputs[0].value < dust_limit {
					let (utxo, size) = self.select_cardinal_utxo(
						(dust_limit - self.outputs[0].value).to_sat(),
						true,
					)?;
	...
	fn add_value(mut self) -> Result<Self> {
		...
		let min_value = match self.target {
			Target::Postage => self.outputs.last().unwrap().script_pubkey.minimal_non_dust(),
			Target::Value(value) | Target::ExactPostage(value) => value,
		};

		let total = min_value.checked_add(estimated_fee).ok_or(Error::ValueOverflow)?;
		...
		if let Some(mut deficit) = total.checked_sub(self.outputs.last().unwrap().value) {
			while deficit > Amount::ZERO {
				...
				let needed = deficit.checked_add(additional_fee).ok_or(Error::ValueOverflow)?;

				let (utxo, value) = self.select_cardinal_utxo(needed.to_sat(), false)?;

				let benefit =
					value.checked_sub(additional_fee).ok_or(Error::NotEnoughCardinalUtxos)?;
	...
	fn strip_value(mut self) -> Self {
		...
		let total_output_amount = self.outputs.iter().map(|tx_out| tx_out.value).sum::<Amount>();
	...
	fn deduct_fee(mut self) -> Self {
		...
		let last_tx_out = self.outputs.last_mut().expect("No output to deduct fee from");
	...
	fn build(self) -> Result<Transaction> {
		...
		for output in &transaction.output {
			if output.script_pubkey == self.recipient {
				...
				assert!(
					self.change_addresses.iter().any(|change_address| change_address
		...
		assert_eq!(actual_fee, expected_fee, "invariant: fee estimation is correct",);
...
mod tests {
	...
	fn select_sat() {
		...
		utxos.remove(1);
		assert_eq!(tx_builder.utxos, utxos.iter().map(|(outpoint, _ranges)| *outpoint).collect());
	...
	fn tx_builder_to_transaction() {
		...
		let tx_builder = TransactionBuilder {
			...
			outputs: vec![
				TxOut { script_pubkey: recipient(), value: Amount::from_sat(5_000) },
				TxOut { script_pubkey: change(0).script_pubkey(), value: Amount::from_sat(5_000) },
				TxOut { script_pubkey: change(1).script_pubkey(), value: Amount::from_sat(1_724) },
			],
	...
	fn insufficient_padding_to_add_postage_small_utxos() {
		let utxos =
			vec![(outpoint(1), tx_out(5_000, address(0))), (outpoint(2), tx_out(1, address(0)))];
	...
	fn invariant_satpoint_outpoint_is_contained_in_utxos() {
		TransactionBuilder::new(
			satpoint(2, 0),
			BTreeMap::new(),
			vec![(outpoint(1), tx_out(4, address(0)))].into_iter().collect(),
			BTreeSet::new(),
			BTreeSet::new(),
			recipient(),
			[change(0), change(1)],
	...
	fn invariant_inputs_spend_sat() {
		TransactionBuilder::new(
			satpoint(1, 2),
			BTreeMap::new(),
			vec![(outpoint(1), tx_out(5, address(0)))].into_iter().collect(),
			BTreeSet::new(),
			BTreeSet::new(),
			recipient(),
			[change(0), change(1)],
	...
	fn alignment_output_under_dust_limit_is_padded() {
		...
		pretty_assert_eq!(
			...
			Ok(Transaction {
				...
				output: vec![tx_out(10_001, change(1)), tx_out(9_811, recipient_address())],
			})
		)
	}
	...
	fn invariant_recipient_appears_exactly_once() {
		...
		TransactionBuilder {
			...
			outputs: vec![
				TxOut { script_pubkey: recipient(), value: Amount::from_sat(5_000) },
				TxOut { script_pubkey: recipient(), value: Amount::from_sat(5_000) },
				TxOut { script_pubkey: change(1).script_pubkey(), value: Amount::from_sat(1_774) },
			],
			target: Target::Postage,
			network: Network::Testnet,
		}
	...
	fn invariant_change_appears_at_most_once() {
		...
		TransactionBuilder {
			...
			outputs: vec![
				TxOut { script_pubkey: recipient(), value: Amount::from_sat(5_000) },
				TxOut { script_pubkey: change(0).script_pubkey(), value: Amount::from_sat(5_000) },
				TxOut { script_pubkey: change(0).script_pubkey(), value: Amount::from_sat(1_774) },
			],
			target: Target::Postage,
			network: Network::Testnet,
		}
	...
	fn do_not_select_outputs_which_do_not_pay_for_their_own_fee_at_default_fee_rate() {
		let utxos =
			vec![(outpoint(1), tx_out(1_000, address(0))), (outpoint(2), tx_out(100, address(0)))];
	...
	fn do_not_select_outputs_which_do_not_pay_for_their_own_fee_at_higher_fee_rate() {
		let utxos =
			vec![(outpoint(1), tx_out(1_000, address(0))), (outpoint(2), tx_out(500, address(0)))];
	...
	fn do_not_strip_excess_value_if_it_would_create_dust() {
		pretty_assert_eq!(
			TransactionBuilder::new(
				satpoint(1, 0),
				BTreeMap::new(),
				vec![(outpoint(1), tx_out(1_000, address(0)))].into_iter().collect(),
				BTreeSet::new(),
				BTreeSet::new(),
				recipient(),
				[change(0), change(1)],
	...
	fn possible_to_create_output_of_exactly_max_postage() {
		pretty_assert_eq!(
			TransactionBuilder::new(
				satpoint(1, 0),
				BTreeMap::new(),
				vec![(outpoint(1), tx_out(20_099, address(0)))].into_iter().collect(),
				BTreeSet::new(),
				BTreeSet::new(),
				recipient(),
				[change(0), change(1)],
	...
	fn do_not_strip_excess_value_if_additional_output_cannot_pay_fee() {
		pretty_assert_eq!(
			TransactionBuilder::new(
				satpoint(1, 0),
				BTreeMap::new(),
				vec![(outpoint(1), tx_out(1_500, address(0)))].into_iter().collect(),
				BTreeSet::new(),
				BTreeSet::new(),
				recipient(),
				[change(0), change(1)],
	...
	fn recipient_address_must_be_unique() {
		pretty_assert_eq!(
			TransactionBuilder::new(
				satpoint(1, 0),
				BTreeMap::new(),
				vec![(outpoint(1), tx_out(1000, address(0)))].into_iter().collect(),
				BTreeSet::new(),
				BTreeSet::new(),
				recipient(),
	...
	fn output_over_value_because_fees_prevent_excess_value_stripping() {
		pretty_assert_eq!(
			TransactionBuilder::new(
				satpoint(1, 0),
				BTreeMap::new(),
				vec![(outpoint(1), tx_out(2000, address(0)))].into_iter().collect(),
				BTreeSet::new(),
				BTreeSet::new(),
				recipient(),
				[change(0), change(1)],
	...
	fn output_over_max_postage_because_fees_prevent_excess_value_stripping() {
		pretty_assert_eq!(
			TransactionBuilder::new(
				satpoint(1, 0),
				BTreeMap::new(),
				vec![(outpoint(1), tx_out(45000, address(0)))].into_iter().collect(),
				BTreeSet::new(),
				BTreeSet::new(),
				recipient(),
				[change(0), change(1)],
	...
	fn select_outgoing_can_select_multiple_utxos() {
		let mut utxos = vec![
			(outpoint(2), tx_out(3_006, address(0))), /* 2. biggest utxo is selected 2nd leaving
			                                           *    us needing 4206 more */
			(outpoint(1), tx_out(3_003, address(0))), /* 1. satpoint is selected 1st leaving us
			                                           *    needing 7154 more */
			(outpoint(5), tx_out(3_004, address(0))),
			(outpoint(4), tx_out(3_001, address(0))), /* 4. smallest utxo >= 1259 is selected
			                                           *    4th, filling deficit */
			(outpoint(3), tx_out(3_005, address(0))), /* 3. next biggest utxo is selected 3rd
			                                           *    leaving us needing 1259 more */
		...
		utxos.remove(0);
		assert_eq!(tx_builder.utxos, utxos.iter().map(|(outpoint, _ranges)| *outpoint).collect());
		assert_eq!(tx_builder.inputs, [outpoint(1), outpoint(2), outpoint(3), outpoint(4)]); // value inputs are pushed at the end
		assert_eq!(
			tx_builder.outputs,
			[TxOut {
				script_pubkey: recipient(),
	...
	fn pad_alignment_output_can_select_multiple_utxos() {
		let mut utxos = vec![
			(outpoint(4), tx_out(101, address(0))), /* 4. smallest utxo >= 84 is selected 4th,
			                                         *    filling deficit */
			(outpoint(1), tx_out(20_000, address(0))), /* 1. satpoint is selected 1st leaving
			                                            *    deficit 293 */
			(outpoint(2), tx_out(105, address(0))), /* 2. biggest utxo <= 293 is selected 2nd
			                                         *    leaving deficit 188 */
			(outpoint(5), tx_out(103, address(0))),
			(outpoint(6), tx_out(10_000, address(0))),
			(outpoint(3), tx_out(104, address(0))), /* 3. biggest utxo <= 188 is selected 3rd
			                                         *    leaving deficit 84 */
		...
		utxos.remove(0);
		assert_eq!(tx_builder.utxos, utxos.iter().map(|(outpoint, _ranges)| *outpoint).collect());
		assert_eq!(tx_builder.inputs, [outpoint(4), outpoint(3), outpoint(2), outpoint(1)]); // padding inputs are inserted at the start
		assert_eq!(
			tx_builder.outputs,
			[
				TxOut {
				...
				TxOut { script_pubkey: recipient(), value: Amount::from_sat(19_999) }
			]
		)
	}
	...
	fn select_cardinal_utxo_prefer_under_helper(
		...
		assert_eq!(
			tx_builder.select_cardinal_utxo(target_value.to_sat(), prefer_under).unwrap().1,
			expected_value
		);
	}
	...
	fn select_cardinal_utxo_ignores_locked_utxos_and_errors_if_none_available() {
		...
		assert_eq!(tx_builder.select_cardinal_utxo(500, false), Err(Error::NotEnoughCardinalUtxos),);
	}

	#[test]
	fn select_cardinal_utxo_ignores_locked_utxos() {
		let utxos =
			vec![(outpoint(1), tx_out(500, address(0))), (outpoint(2), tx_out(500, address(0)))];
		let locked_utxos = vec![outpoint(1)];
		...
		assert_eq!(tx_builder.select_cardinal_utxo(500, false).unwrap().0, outpoint(2),);
	}

	#[test]
	fn prefer_further_away_utxos_if_they_are_newly_under_target() {
		let utxos =
			vec![(outpoint(1), tx_out(510, address(0))), (outpoint(2), tx_out(400, address(0)))];
		...
		assert_eq!(tx_builder.select_cardinal_utxo(500, true).unwrap().0, outpoint(2),);
	}

	#[test]
	fn prefer_further_away_utxos_if_they_are_newly_over_target() {
		let utxos =
			vec![(outpoint(1), tx_out(490, address(0))), (outpoint(2), tx_out(600, address(0)))];
