...
#[derive(Clone)]
pub(crate) struct WalletBrc721Constructor {
...
impl WalletBrc721Constructor {
	pub(crate) fn construct(
		name: String,
		no_sync: bool,
		settings: Settings,
		rpc_url: Url,
	) -> Result<WalletBrc721<'static>> {
	...
	pub(crate) fn build(self) -> Result<WalletBrc721<'static>> {
		let database = WalletBrc721::open_database(&self.name, &self.settings)?;

		let bitcoin_client = {
			let client = WalletBrc721::check_version(
				self.settings.bitcoin_rpc_client(Some(self.name.clone()))?,
			)?;
			...
			if !client.list_wallets()?.contains(&self.name) {
				loop {
					match client.load_wallet(&self.name) {
						...
						)) if err.code == -4 && err.message == "WalletBrc721 already loading." => {
							// wallet loading
							eprint!(".");
			...
			if client.get_wallet_info()?.private_keys_enabled {
				WalletBrc721::check_descriptors(
					&self.name,
					client
						.call::<ListDescriptorsResult>(
							"listdescriptors",
		...
		// This creates a heap-allocated wallet and leaks it so it has a 'static lifetime.
		// While this creates a memory leak, it's a common pattern for objects that need to live for
		// the entire program duration.
		let wallet = Box::leak(Box::new(wallet));
		Ok(WalletBrc721::from_wallet(wallet))
	}
	...
	) -> Result<(BTreeMap<SatPoint, Vec<InscriptionId>>, BTreeMap<InscriptionId, api::Inscription>)>
	{
	...
	fn get_utxos(bitcoin_client: &Client) -> Result<BTreeMap<OutPoint, TxOut>> {
		Ok(bitcoin_client
			...
			.map(|utxo| {
				...
				let txout = TxOut { script_pubkey: utxo.script_pub_key, value: utxo.amount };
	...
	fn get_locked_utxos(bitcoin_client: &Client) -> Result<BTreeMap<OutPoint, TxOut>> {
		...
		for outpoint in outpoints {
			let Some(tx_out) =
				bitcoin_client.get_tx_out(&outpoint.txid, outpoint.vout, Some(false))?
			else {
				continue;
			};
	...
	pub fn get(&self, path: &str) -> Result<reqwest::blocking::Response> {
		self.ord_client.get(self.rpc_url.join(path)?).send().map_err(|err| anyhow!(err))
	}
