...
use wallet::transaction_builder::Target;
...
mod tests {
	use super::*;
	use crate::wallet::batch::{self, ParentInfo};
	...
	fn reveal_transaction_pays_fee() {
		...
		let batch::Transactions { commit_tx, reveal_tx, .. } = batch::Plan {
	...
	fn inscribe_with_no_satpoint_and_no_cardinal_utxos() {
		...
		inscriptions.insert(SatPoint { outpoint: outpoint(1), offset: 0 }, vec![inscription_id(1)]);
		...
		assert!(error.contains("wallet contains no cardinal utxos"), "{}", error);
	}
	...
	fn inscribe_with_custom_fee_rate() {
		...
		let reveal_value =
			commit_tx.output.iter().map(|o| o.value).reduce(|acc, i| acc + i).unwrap();

		assert_eq!(reveal_value.to_sat(), 20_000 - fee);

		let fee = FeeRate::try_from(fee_rate).unwrap().fee(reveal_tx.vsize()).to_sat();
	...
	fn inscribe_with_parent() {
		...
		let parent_info = ParentInfo {
			...
			location: SatPoint { outpoint: outpoint(1), offset: 0 },
			tx_out: TxOut {
		...
		let child_inscription =
			InscriptionTemplate { parents: vec![parent_inscription], ..default() }.into();
		...
		let sig_vbytes = 16;
		let fee = FeeRate::try_from(fee_rate).unwrap().fee(reveal_tx.vsize() + sig_vbytes);

		assert_eq!(fee, commit_tx.output[0].value - reveal_tx.output[1].value);
		assert_eq!(reveal_tx.output[0].script_pubkey, parent_info.destination.script_pubkey());
	...
	fn batch_inscribe_with_parent() {
		...
		let inscriptions = vec![
			InscriptionTemplate { parents: vec![parent], ..default() }.into(),
			InscriptionTemplate { parents: vec![parent], ..default() }.into(),
			InscriptionTemplate { parents: vec![parent], ..default() }.into(),
		];
	...
	fn batch_inscribe_satpoints_with_parent() {
		...
		let parent_info = ParentInfo {
			destination: change(3),
			id: parent,
			location: SatPoint { outpoint: outpoint(4), offset: 0 },
			tx_out: TxOut {
				script_pubkey: change(0).script_pubkey(),
		...
		let commit_address = change(1);
		let reveal_addresses = vec![recipient_address(), recipient_address(), recipient_address()];

		let inscriptions = vec![
			InscriptionTemplate { parents: vec![parent], pointer: Some(10_000) }.into(),
			InscriptionTemplate { parents: vec![parent], pointer: Some(11_111) }.into(),
			InscriptionTemplate { parents: vec![parent], pointer: Some(13_3333) }.into(),
		];

		let reveal_satpoints = utxos
			.iter()
			.take(3)
			.map(|(outpoint, txout)| (SatPoint { outpoint: *outpoint, offset: 0 }, txout.clone()))
		...
		.create_batch_transactions(
			wallet_inscriptions,
			Chain::Signet,
			reveal_satpoints.iter().map(|(satpoint, _)| satpoint.outpoint).collect(),
			BTreeSet::new(),
			utxos.into_iter().collect(),
			[commit_address, change(2)],
			change(3),
	...
	fn batch_inscribe_into_separate_outputs() {
		...
		assert!(reveal_tx.output.iter().all(|output| output.value == TARGET_POSTAGE));
	}
	...
	fn batch_inscribe_into_separate_outputs_with_parent() {
		...
		assert_eq!(vec![parent], ParsedEnvelope::from_transaction(&reveal_tx)[0].payload.parents(),);
		assert_eq!(vec![parent], ParsedEnvelope::from_transaction(&reveal_tx)[1].payload.parents(),);
