...
impl File {
	pub(crate) fn load(path: &Path) -> Result<Self> {
		...
		ensure!(!sat_and_satpoint, "batchfile cannot set both `sat` and `satpoint`",);
		...
		if batchfile.inscriptions.iter().any(|entry| entry.destination.is_some()) &&
		...
		let any_entry_has_satpoint =
			batchfile.inscriptions.iter().any(|entry| entry.satpoint.is_some());
        ...
        "if `satpoint` is set for any inscription, then all inscriptions need to specify a satpoint"
				...
				batchfile.inscriptions.iter().all(|entry| entry.satpoint.unwrap().offset == 0),
				"`satpoint` can only be specified for first sat of an output"
			);
		}

		if batchfile.mode == Mode::SatPoints {
			ensure!(batchfile.postage.is_none(), "`postage` cannot be set if in `satpoints` mode");

			ensure!(batchfile.sat.is_none(), "`sat` cannot be set if in `satpoints` mode");

			ensure!(batchfile.satpoint.is_none(), "`satpoint cannot be set if in `satpoints` mode");
	...
	pub(crate) fn inscriptions(
		...
		compress: bool,
	) -> Result<(Vec<Inscription>, Vec<(SatPoint, TxOut)>, Vec<Amount>, Vec<Address>)> {
		...
		for (i, entry) in self.inscriptions.iter().enumerate() {
			...
			inscriptions.push(Inscription::new(
				...
				entry.file.clone(),
				Some(pointer),
				self.etching.and_then(|etch| (i == 0).then_some(etch.rune.rune)),
			)?);

			let postage = if self.mode == Mode::SatPoints {
				let satpoint =
					entry.satpoint.ok_or_else(|| anyhow!("no satpoint specified for entry {i}"))?;
...
"#,
		...
		assert_eq!(
			batch::File::load(batch_file.as_path()).unwrap_err().to_string(),
			"specifying `satpoint` in an inscription only works in `satpoints` mode"
		);
	}
			...
			batch::File {
				...
				etching: Some(Etching {
					...
					terms: Some(batch::Terms {
						...
						cap: 90,
						height: Some(batch::Range { start: Some(840000), end: Some(850000) }),
						offset: Some(batch::Range { start: Some(1000), end: Some(9000) }),
					}),
					turbo: true,
				}),
				inscriptions: vec![
