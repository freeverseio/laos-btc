...
impl Plan {
	pub(crate) fn inscribe(
		...
		let Transactions { commit_tx, commit_vout, reveal_tx, recovery_key_pair, total_fees, rune } =
			...
			let commit_psbt = wallet
				...
				.wallet_process_psbt(
					&base64::engine::general_purpose::STANDARD.encode(
						Psbt::from_unsigned_tx(Self::remove_witnesses(commit_tx.clone()))?
							.serialize(),
					),
					Some(false),
					None,
		...
		ensure!(result.complete, format!("Failed to sign reveal transaction: {:?}", result.errors));
		...
		let commit_txid = wallet.bitcoin_client().send_raw_transaction(&signed_commit_tx)?;
		...
		if let Some(ref rune_info) = rune {
			...
			Ok(Some(Box::new(wallet.wait_for_maturation(rune_info.rune.rune)?)))
		} else {
	...
	fn output(
		...
		for i in 0..inscriptions.len() {
			...
			let offset = match self.mode {
				Mode::SharedOutput =>
					self.postages[0..i].iter().map(|amount| amount.to_sat()).sum(),
			...
			inscriptions_output.push(InscriptionInfo {
				...
				location: SatPoint { outpoint: OutPoint { txid: reveal, vout }, offset },
			});
		}
	...
	pub(crate) fn create_batch_transactions(
		...
		for inscription in &self.inscriptions {
			assert_eq!(
				inscription.parents(),
				self.parent_info.iter().map(|info| info.id).collect::<Vec<InscriptionId>>()
			);
		}

		match self.mode {
			Mode::SameSat => {
				assert_eq!(self.postages.len(), 1, "invariant: same-sat has only one postage");
				assert_eq!(
		...
		let satpoint = if let Some(satpoint) = self.satpoint {
			...
			utxos
				...
				.map(|(outpoint, _amount)| SatPoint { outpoint: *outpoint, offset: 0 })
		...
		for ParentInfo { location, id: _, destination, tx_out } in &self.parent_info {
			reveal_inputs.push(location.outpoint);
			reveal_outputs
				.push(TxOut { script_pubkey: destination.script_pubkey(), value: tx_out.value });
		}
		...
		if let Some(etching) = self.etching {
			...
			if premine > 0 {
				...
				reveal_outputs
					.push(TxOut { script_pubkey: reveal_change.into(), value: TARGET_POSTAGE });
			...
			reveal_outputs.push(TxOut { script_pubkey, value: Amount::from_sat(0) });
		...
		reveal_inputs[commit_input] =
			OutPoint { txid: unsigned_commit_tx.compute_txid(), vout: vout.try_into().unwrap() };
		...
		witness.push(Signature { signature, sighash_type: TapSighashType::Default }.to_vec());
		...
		match (Runestone::decipher(&reveal_tx), runestone) {
			...
			(Some(_), None) => {
				panic!("commit transaction contained runestone, but none was expected")
			},
			(None, Some(_)) => {
		...
		let rune = rune.map(|(destination, rune, vout)| RuneInfo {
			...
			location: vout.map(|vout| OutPoint { txid: reveal_tx.compute_txid(), vout }),
			rune,
		});
	...
	fn backup_recovery_key(wallet: &Wallet, recovery_key_pair: TweakedKeypair) -> Result {
		let recovery_private_key =
			PrivateKey::new(recovery_key_pair.to_inner().secret_key(), wallet.chain().network());

		let info = wallet
			.bitcoin_client()
			.get_descriptor_info(&format!("rawtr({})", recovery_private_key.to_wif()))?;

		let response = wallet.bitcoin_client().import_descriptors(ImportDescriptors {
			descriptor: format!(
				"rawtr({})#{}",
		...
		let fee = {
			...
			for (current_index, txin) in reveal_tx.input.iter_mut().enumerate() {
				...
				if current_index == commit_input_index {
					txin.witness.push(
						Signature::from_slice(&[0; SCHNORR_SIGNATURE_SIZE]).unwrap().to_vec(),
					);
