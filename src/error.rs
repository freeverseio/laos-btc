...
pub enum SnafuError {
	...
	AddressParse { source: bitcoin::address::error::ParseError, input: String },
	#[snafu(display("Failed to parse hash `{}`", input))]
	HashParse { source: bitcoin::hex::HexToArrayError, input: String },
	#[snafu(display("Failed to parse inscription ID `{}`", input))]
	InscriptionIdParse { source: inscriptions::inscription_id::ParseError, input: String },
	#[snafu(display("Failed to parse integer `{}`", input))]
	IntegerParse { source: std::num::ParseIntError, input: String },
	#[snafu(display("Failed to parse out point `{}`", input))]
	OutPointParse { source: bitcoin::transaction::ParseOutPointError, input: String },
	#[snafu(display("Failed to parse rune `{}`", input))]
	RuneParse { source: ordinals::spaced_rune::Error, input: String },
	#[snafu(display("Failed to parse sat `{}`", input))]
	SatParse { source: ordinals::sat::Error, input: String },
	#[snafu(display("Failed to parse sat point `{}`", input))]
	SatPointParse { source: ordinals::sat_point::Error, input: String },
	...
	AmountParse { source: <Amount as FromStr>::Err, input: String },
	...
	AddressConversion { source: bitcoin::address::FromScriptError },
	#[snafu(display("{err}"))]
	Anyhow { err: anyhow::Error },
	#[snafu(display("environment variable `{variable}` not valid unicode: `{}`", value.to_string_lossy()))]
	EnvVarUnicode { backtrace: Backtrace, value: OsString, variable: String },
	#[snafu(display("I/O error at `{}`", path.display()))]
	Io { backtrace: Backtrace, path: PathBuf, source: io::Error },
