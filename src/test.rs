...
pub(crate) use mockcore::TransactionTemplate;
...
pub(crate) use unindent::Unindent;
...
pub(crate) fn outpoint(n: u32) -> OutPoint {
	OutPoint { txid: txid(n), vout: n }
}

pub(crate) fn satpoint(n: u32, offset: u64) -> SatPoint {
	SatPoint { offset, outpoint: outpoint(n) }
}
...
pub(crate) fn tx_out(value: u64, address: Address) -> TxOut {
	TxOut { value: Amount::from_sat(value), script_pubkey: address.script_pubkey() }
}
...
pub(crate) fn default_address(chain: Chain) -> Address {
	Address::from_script(&ScriptBuf::new_p2wpkh(&WPubkeyHash::all_zeros()), chain.network())
		.unwrap()
}
