...
impl Chain {
	...
	pub(crate) fn genesis_coinbase_outpoint(self) -> OutPoint {
		OutPoint { txid: self.genesis_block().coinbase().unwrap().compute_txid(), vout: 0 }
	}
...
impl FromStr for Chain {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match s {
			...
			_ => Err(SnafuError::InvalidChain { chain: s.to_string() }),
		}
	}
}
...
mod tests {
	...
	fn from_str() {
		...
		assert_eq!("foo".parse::<Chain>().unwrap_err().to_string(), "Invalid chain `foo`");
	}
}
