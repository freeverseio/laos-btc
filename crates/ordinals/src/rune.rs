...
mod tests {
	...
	fn mainnet_minimum_at_height() {
		...
		fn case(height: u32, minimum: &str) {
			...
			assert_eq!(Rune::minimum_at_height(Network::Bitcoin, Height(height)), minimum,);
		...
		case(START + Rune::UNLOCK_INTERVAL * 10 + Rune::UNLOCK_INTERVAL / 2, "NA");
		...
		case(START + Rune::UNLOCK_INTERVAL * 11 + Rune::UNLOCK_INTERVAL / 2, "N");
	...
	fn reserved_rune_unlock_height() {
		...
		assert_eq!(Rune(Rune::RESERVED + 1).unlock_height(Network::Bitcoin), None);
		assert_eq!(Rune(Rune::RESERVED - 1).unlock_height(Network::Bitcoin), Some(Height(0)));
	}
	...
	fn unlock_height() {
		...
		fn case(rune: &str, unlock_height: u32) {
			...
			if unlock_height > 0 {
				...
				assert!(
					rune < Rune::minimum_at_height(Network::Bitcoin, Height(unlock_height - 1))
				);
			}
		}
		...
		for i in 0..4 {
			for n in Rune::STEPS[i]..Rune::STEPS[i + 1] {
				...
				assert!(
					rune < Rune::minimum_at_height(Network::Bitcoin, Height(unlock_height.0 - 1))
				);
			}
		}
	}
