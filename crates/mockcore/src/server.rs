...
impl Api for Server {
	...
	fn get_block_header(
		...
		if verbose {
			let height = match self.state().hashes.iter().position(|hash| *hash == block_hash) {
				Some(height) => height,
				None => return Err(Self::not_found()),
			};

			Ok(serde_json::to_value(GetBlockHeaderResult {
				bits: String::new(),
			...
			match self.state().blocks.get(&block_hash) {
				Some(block) =>
					Ok(serde_json::to_value(hex::encode(serialize(&block.header))).unwrap()),
				None => Err(Self::not_found()),
			}
		}
	}
	...
	fn create_wallet(
		...
		Ok(LoadWalletResult { name, warning: None })
	}
	...
	fn fund_raw_transaction(
		...
		let mut transaction = Transaction { version, input, output, lock_time };

		assert_eq!(options.change_position, Some(transaction.output.len().try_into().unwrap()));

		let mut state = self.state();

		let output_value = transaction.output.iter().map(|txout| txout.value.to_sat()).sum();
		...
		if let Some(utxos) = &utxos {
			...
			if utxos[0].amount == Some(Amount::ZERO) {
				transaction.input[0].witness =
					self.state().wallet.sign_bip322(&utxos[0], &transaction);
			}
		}
	...
	fn send_raw_transaction(
		...
		assert!(maxfeerate.is_none(), "sendrawtransaction: maxfeerate is not supported");
	...
	fn send_to_address(
		...
		let mut transaction = Transaction {
			...
			output: vec![
				TxOut { value, script_pubkey: address.assume_checked_ref().script_pubkey() },
				TxOut {
	...
	fn get_transaction(
		...
		Ok(serde_json::to_value(GetTransactionResult {
			info: WalletTxInfo {
				txid,
				confirmations: confirmations.unwrap().try_into().unwrap(),
				time: 0,
	...
	fn get_raw_transaction(
		...
		let blockhash =
			tx_height.map(|tx_height| state.hashes[usize::try_from(*tx_height).unwrap()]);

		if verbose.unwrap_or(false) {
			match state.transactions.get(&txid) {
				Some(transaction) => Ok(serde_json::to_value(GetRawTransactionResult {
	...
	fn import_descriptors(
		...
		self.state()
			.descriptors
			.extend(req.into_iter().map(|params| (params.descriptor, params.timestamp)));

		Ok(vec![ImportMultiResult { success: true, warnings: Vec::new(), error: None }])
	}
	...
	fn lock_unspent(
		...
		for output in outputs {
			let output = OutPoint { vout: output.vout, txid: output.txid };
	...
	fn load_wallet(&self, wallet: String) -> Result<LoadWalletResult, jsonrpc_core::Error> {
		if self.state().wallets.contains(&wallet) {
			...
			Ok(LoadWalletResult { name: wallet, warning: None })
		} else {
			Err(Self::not_found())
		}
	}
	...
	fn list_wallets(&self) -> Result<Vec<String>, jsonrpc_core::Error> {
		Ok(self.state().loaded_wallets.clone().into_iter().collect::<Vec<String>>())
	}
	...
	fn wallet_process_psbt(
		...
		let mut psbt =
			Psbt::deserialize(&base64::engine::general_purpose::STANDARD.decode(psbt).unwrap())
				.unwrap();

		for (i, txin) in psbt.unsigned_tx.input.iter().enumerate() {
			psbt.inputs[i].witness_utxo = Some(
				self.state().transactions.get(&txin.previous_output.txid).unwrap().output
	...
	fn finalize_psbt(
		...
		Ok(FinalizePsbtResult { psbt: None, hex: Some(serialize(&transaction)), complete: true })
	}
}
