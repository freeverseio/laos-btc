...
impl Wallet {
	...
	pub fn new_address(&mut self) -> Address {
		let address = {
			let derived_key = self
				...
				.derive_priv(
					&self.secp,
					&self.derivation_path.child(ChildNumber::Normal { index: self.next_index }),
				)
				.unwrap();
		...
		self.address_indices.insert(address.clone(), self.next_index);
		self.next_index += 1;
	...
	pub fn sign_bip322(
		...
		let private_key = self.master_key.derive_priv(&self.secp, &derivation_path).unwrap();
		...
		witness.push(bitcoin::taproot::Signature { signature, sighash_type }.to_vec());
