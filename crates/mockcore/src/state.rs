...
impl State {
	...
	pub(crate) fn new_address(&mut self, change: bool) -> Address {
		...
		if change { &mut self.change_addresses } else { &mut self.receive_addresses }
			.push(address.clone());
	...
	pub fn balances(&self) -> BTreeMap<Address, Vec<(OutPoint, Amount)>> {
		...
		for (&outpoint, &amount) in &self.utxos {
			...
			addresses.entry(address).or_default().push((outpoint, amount));
		}
	...
	pub(crate) fn mine_block(&mut self, subsidy: u64) -> Block {
		...
		self.transactions.insert(coinbase.compute_txid(), coinbase.clone());
		...
		let block = Block {
			...
			txdata: std::iter::once(coinbase).chain(self.mempool.drain(0..)).collect(),
		};
		...
		for tx in block.txdata.iter() {
			...
			for (vout, txout) in tx.output.iter().enumerate() {
				if !txout.script_pubkey.is_op_return() {
					self.utxos.insert(
						OutPoint { txid: tx.compute_txid(), vout: vout.try_into().unwrap() },
						txout.value,
					);
				}
			}
	...
	pub(crate) fn broadcast_tx(&mut self, template: TransactionTemplate) -> Txid {
		...
		if template.outputs > 0 {
			assert_eq!(value_per_output * template.outputs as u64 + template.fee, total_value);
		}
			...
			output: (0..template.outputs)
				.map(|i| TxOut {
					value: Amount::from_sat(
						template.output_values.get(i).cloned().unwrap_or(value_per_output),
					),
