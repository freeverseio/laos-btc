...
pub fn builder() -> Builder {
	Builder { fail_lock_unspent: false, network: Network::Bitcoin, version: 250000 }
}
...
impl Builder {
	...
	pub fn build(self) -> Handle {
		let state =
			Arc::new(Mutex::new(State::new(self.network, self.version, self.fail_lock_unspent)));
		...
		Handle { close_handle: Some(close_handle), tempdir, port, state }
	}
}
...
impl From<OutPoint> for JsonOutPoint {
	fn from(outpoint: OutPoint) -> Self {
		Self { txid: outpoint.txid, vout: outpoint.vout }
	}
}
...
impl Handle {
	...
	pub fn address(&self, output: OutPoint) -> Address {
		...
		Address::from_script(
			&state.transactions.get(&output.txid).unwrap().output[output.vout as usize]
				.script_pubkey,
			state.network,
		)
		.unwrap()
	...
	pub fn tx_by_id(&self, txid: Txid) -> Transaction {
		self.state().transactions.get(&txid).expect("unknown transaction").clone()
	}
